						...
						<select class="form-control custom-control" id="trans_idPeminjam" name="trans_idPeminjam" onChange="getValPeminjam(this)">
							...
							<?php
								...
								$sql = "SELECT trans_id, nomor_surat FROM bbm_kapaltrans WHERE kapal_code_temp = (
											...
											) AND status_ba = 10 AND status_temp = 1";
			...
			<div class="row">
				...
				<div class="col-md-4">
				 ...
				 <div class="col-md-5">
					...
					<input type="text" name="volume_pemakaian" id="volume_pemakaian" class="form-control angka" value="0" readonly="">
				 </div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_pengembalian_pinjaman_bbm/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_pinjaman_bbm/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_pinjaman_bbm/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_pinjaman_bbm/getDataLihat/'+trans_id);
	$('#tab5').click();
}
	...
	if(tanggal_surat == ''){
		...
		$.ajax({
			...
			success: function (data) {	
				...
				$("#volume_pemakaian").val(format_rupiah(data.volume_pemakaian));
...
function validateSebelum(){
	...
	if( tanggal_surat <= tanggal_peminjaman){
	
		alert("Tanggal Pengembalian harus lebih dari tanggal pinjam!!");
		clear_peminjam();
		...
		if(volume_pemakaian > volume_sebelum){
			...
			alert("Volume Sebelum Tidak Cukup Melakukan Pengembalian Silahkan Sounding Ulang!!");
			clear_peminjam();
...
function clear_peminjam(){
	...
	document.getElementById("an_nakhoda_temp").checked = false;
	document.getElementById("an_kkm_temp").checked = false;
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_pengembalian_pinjaman_bbm/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_pengembalian_pinjaman_bbm/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
