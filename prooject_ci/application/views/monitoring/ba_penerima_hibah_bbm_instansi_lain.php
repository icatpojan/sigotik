			...
			<div class="row">
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="kapal_code_temp" id="kapal_code_temp" class="form-control" value="">
					</div>
				</div>
			...
			<div class="row">
			    ...
			    <div class="col-md-5">
				 ...
				 <div class="col-md-5">
					...
					<input type="text" name="volume_pengisian" id="volume_pengisian" class="form-control angka" value="" onkeypress="validate(event)" onclick="validateOnClik(0)" onChange="validateVolume(0)">
				 </div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_bbm_instansi_lain/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_bbm_instansi_lain/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_bbm_instansi_lain/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_bbm_instansi_lain/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function getVolumeSounding(){
    ...
    $.ajax({
		type : "POST",
		url	: "<?php echo base_url()?>index.php/monitoring/ba_peminjaman_bbm/getDataBa",
		data: dataForm,
		dataType: "json",
		success: function (data) {
...
function validateVolume(x){
	...
	if(x==1){
		...
		var volume_pengisian = parseFloat(document.getElementById('volume_pengisianEd').value.replace(/,/g ,''));
		
		if(volume_pengisian == 0){
			
			var volume_sisa = volume_sebelum;
		}else{
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_penerima_hibah_bbm_instansi_lain/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_penerima_hibah_bbm_instansi_lain/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
