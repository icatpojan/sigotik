...
@section('title', 'BA Penerimaan Bbm')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">BA Penerimaan Bbm</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola Berita Acara Penerimaan Bbm</p>
        </div>
   ...
   `);
        ...
        function setDefaultDates() {
            ...
            try {
                ...
                // Field tanggal_sebelum dan tanggal_pengisian sudah tidak ada di BA Penerimaan BBM
        ...
        function resetForm() {
            ...
            try {
                ...
                if ($('#modalTitle').length > 0) {
                    $('#modalTitle').text('Form Tambah BA Penerimaan BBM');
                }
        ...
        function loadData() {
            ...
            $.ajax({
                url: '{{ route("ba-penerimaan-bbm.data") }}'
                , type: 'GET'
                , data: params
                , beforeSend: function() {
                    showLoading();
        ...
        function loadKapalData(kapalId) {
            $.ajax({
                url: '{{ route("ba-penerimaan-bbm.kapal-data") }}'
                , type: 'GET'
                , data: {
                    kapal_id: kapalId
                }
        ...
        function loadBaData(kapalId) {
            ...
            $.ajax({
                url: '{{ route("ba-penerimaan-bbm.ba-data") }}'
                , type: 'GET'
                , data: {
                    kapal_id: kapalId
        ...
        function clearKapalData() {
            ...
            try {
                ...
                $('#nama_kkm').val('');
                $('#nip_kkm').val('');
                // Clear field baru untuk BA Penerimaan BBM
                $('#penyedia').val('');
                $('#no_so').val('');
                // Field volume_sebelum dan tanggal_sebelum sudah tidak ada di BA Penerimaan BBM
       ...
       <div class="mt-6">
           <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4">
               ...
               <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                   ...
                   <div class="flex justify-between">
                       <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Volume Penerimaan Bbm:</span>
       ...
       `;
        ...
        function fillEditForm(data) {
            ...
            $('#baForm')[0].reset();
            $('#modalTitle').text('Form Edit BA Penerimaan BBM');
            ...
            // Field baru untuk BA Penerimaan BBM
        ...
        function setupEventHandlers() {
            ...
            $('#deleteDocumentBtn').on('click', function() {
                if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                    $.ajax({
                        url: `/ba-penerimaan-bbm/${currentBaId}/delete-document`
                        , type: 'DELETE'
                        , data: {
                            _token: '{{ csrf_token() }}'
                        }
        ...
        function handleFormSubmit(e) {
            ...
            let url = '{{ route("ba-penerimaan-bbm.store") }}';
            let method = 'POST';

            if (currentEditMode) {
                url = `{{ url('ba-penerimaan-bbm') }}/${currentBaId}`;
        ...
        function handleUploadSubmit(e) {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-bbm') }}/${currentBaId}/upload`
                , type: 'POST'
                , data: formData
                , processData: false
                , contentType: false
        ...
        function handleViewDocument() {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-bbm') }}/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
        ...
        function deleteDocument(baId) {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-bbm') }}/${baId}/delete-document`
            ...
            $.ajax({
                ...
                , success: function(response) {
                    if (response.success) {
                        ...
                        console.log('Transdetails:', response.data.transdetails);
                        fillEditForm(response.data);
                        currentBaId = id;
                        currentEditMode = true;
                        $('#modalTitle').text('Edit BA Penerimaan BBM');
                        $('#submitBtn').html('<i class="fas fa-save me-2"></i>Update BA');
                        $('#baModal').removeClass('hidden');
                    } else {
        ...
        window.generatePdf = function(baId) {
            ...
            $.ajax({
                url: `/ba-penerimaan-bbm/${baId}/pdf`
                , type: 'GET'
        ...
        window.viewDocument = function(baId) {
            ...
            $.ajax({
                url: `/ba-penerimaan-bbm/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        $('#documentViewer').html(`
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="no_so" name="no_so" required placeholder="SO/001/2024" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
    ...
    <div class="relative mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 mt-10 mb-10 max-h-[90vh] overflow-y-auto help-modal-scroll">
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between pb-4">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">Panduan BA Penerimaan BBM</h3>
            ...
            <div class="space-y-6">
                <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                    <h4 class="text-lg font-semibold text-blue-900 dark:text-blue-100 mb-3">Tentang BA Penerimaan BBM</h4>
                    <p class="text-blue-800 dark:text-blue-200 text-sm leading-relaxed">
                        Berita Acara Penerimaan BBM digunakan untuk mencatat penerimaan BBM dari penyedia.
                        Dokumen ini berisi informasi tentang volume BBM yang diterima, penyedia, dan kondisi penerimaan.
                    </p>
                </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Pilih kapal yang akan menerima BBM. Data kapal akan otomatis terisi.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Lengkapi nomor surat, tanggal, jam, zona waktu, lokasi, dan nomor SO.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                <h5 class="font-medium text-gray-900 dark:text-white">Data BBM</h5>
                                <p class="text-sm text-gray-600 dark:text-gray-400">Isi volume pengisian, jenis BBM, dan informasi penyedia BBM.</p>
                            </div>
                        </div>
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                    ...
                    <ul class="text-yellow-800 dark:text-yellow-200 text-sm space-y-1">
                        ...
                        <li>• Nomor SO ditempatkan di section "Lokasi dan Waktu"</li>
                    </ul>
                </div>
            </div>
        </div>
