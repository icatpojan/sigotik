...
@section('title', 'BA Penerimaan Pinjaman BBM')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">BA Penerimaan Pinjaman BBM</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola Berita Acara Penerimaan Pinjaman BBM</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-none border border-gray-200 dark:border-gray-700 overflow-hidden p-6">
        ...
        <div class="overflow-x-auto">
            <table id="baTable" class="min-w-full border-collapse border border-gray-300 dark:border-gray-600" style="border-radius:20%">
...
<script>
    $(document).ready(function() {
        ...
        function setDefaultDates() {
            ...
            console.log('setDefaultDates called, today:', today, 'currentTime:', currentTime);

            // Pastikan elemen tanggal ada sebelum mengatur nilainya
            if ($('#tanggal_surat').length) {
                $('#tanggal_surat').val(today);
                console.log('tanggal_surat set to:', $('#tanggal_surat').val());
            }
            if ($('#jam_surat').length) {
                $('#jam_surat').val(currentTime);
                console.log('jam_surat set to:', $('#jam_surat').val());
            }
        ...
        // Fungsi untuk load data kapal hanya untuk field yang kosong (untuk edit mode)
            ...
            $.ajax({
                ...
                , success: function(response) {
                    if (response.success) {
                        ...
                        // Hanya isi field yang kosong atau belum ada
        ...
        function loadBaPeminjamanDetails(transId) {
            ...
            $.ajax({
                ...
                , success: function(response) {
                    console.log('BA Peminjaman Details Response:', response);
                    if (response.success && response.data) {
                        ...
                        console.log('No data found or response failed');
                    }
                }
                , error: function(xhr) {
       ...
       `;
        ...
        function fillEditForm(data) {
            ...
            // Load data dari database BA terlebih dahulu (prioritas utama)
            ...
            // Load data dari kapal hanya untuk field yang kosong atau belum ada
            // Ini dilakukan setelah data dari database BA sudah di-load
        ...
        function setupEventHandlers() {
            ...
            $('#trans_id_peminjam').on('change', function() {
                ...
                console.log('BA Peminjaman changed, transId:', transId);
                if (transId) {
            ...
            // Form submission - HANYA SATU EVENT HANDLER
            ...
            $('#createBaBtn').on('click', function() {
                console.log('Create BA button clicked');
                resetForm();
                $('#baModal').removeClass('hidden');

                // Set default tanggal, jam dan zona waktu setelah modal terbuka
                setTimeout(function() {
                    const today = new Date().toISOString().split('T')[0];
                    const now = new Date();
                    const currentTime = now.toTimeString().slice(0, 5); // HH:MM format

                    console.log('Setting dates after modal open:', today, currentTime);
                    $('#tanggal_surat').val(today);
                    $('#jam_surat').val(currentTime);
                    $('#zona_waktu_surat').val('WIB');

                    console.log('tanggal_surat after set:', $('#tanggal_surat').val());
                    console.log('jam_surat after set:', $('#jam_surat').val());
                }, 100);
            });
            ...
            $('#deleteDocumentBtn').on('click', function() {
                if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                    $.ajax({
                        url: `/ba-penerimaan-pinjaman-bbm/${currentBaId}/delete-document`
                        , type: 'DELETE'
                        , data: {
                            _token: '{{ csrf_token() }}'
                        }
        ...
        function handleFormSubmit(e) {
            ...
            let url = '{{ route("ba-penerimaan-pinjaman-bbm.store") }}';
            let method = 'POST';

            if (currentEditMode) {
                url = `{{ url('ba-penerimaan-pinjaman-bbm') }}/${currentBaId}`;
        ...
        function handleUploadSubmit(e) {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-pinjaman-bbm') }}/${currentBaId}/upload`
                , type: 'POST'
                , data: formData
                , processData: false
                , contentType: false
        ...
        function handleViewDocument() {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-pinjaman-bbm') }}/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
        ...
        function deleteDocument(baId) {
            ...
            $.ajax({
                url: `{{ url('ba-penerimaan-pinjaman-bbm') }}/${baId}/delete-document`
        ...
        window.generatePdf = function(baId) {
            ...
            $.ajax({
                url: `/ba-penerimaan-pinjaman-bbm/${baId}/pdf`
                , type: 'GET'
        ...
        window.viewDocument = function(baId) {
            ...
            $.ajax({
                url: `/ba-penerimaan-pinjaman-bbm/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        $('#documentViewer').html(`
                            <div class="text-center">
                                <div class="mb-4">
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Dokumen pendukung BA Penerimaan Pinjaman BBM</p>
                        </div>
        ...
        function initDataTable_DELETED() {
        ...
        function loadBaPeminjamanDetails(transId) {
            ...
            $.ajax({
                ...
                , success: function(response) {
                    console.log('BA Peminjaman Details Response (2):', response);
                    if (response.success && response.data) {
                        const data = response.data;
                        console.log('Filling data (2):', data);
                        $('#link_modul_temp').val(data.nomor_surat);
                        $('#volume_sebelum').val(data.volume_sisa);
                        $('#keterangan_jenis_bbm').val(data.keterangan_jenis_bbm);
                        $('#sebab_temp').val(data.sebab_temp);
                        ...
                        console.log('No data found or response failed (2)');
                    }
                }
                , error: function(xhr) {
                    console.error('Error loading BA Peminjaman details:', xhr);
        ...
        // Form submission handler sudah ada di atas (handleFormSubmit)
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="modalTitle" class="text-xl font-semibold text-gray-900 dark:text-white">Form Tambah BA Penerimaan Pinjaman BBM</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut untuk membuat Berita Acara Penerimaan Pinjaman BBM</p>
                    </div>
                ...
                <form id="baForm" class="mt-6 space-y-6">
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="trans_id_peminjam" name="trans_id_peminjam" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-colors">
                                    <option value="">-- Pilih BA Peminjaman --</option>
                                </select>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Pilih BA Peminjaman BBM yang akan diterima</p>
                            </div>
                            <div>
                                <label for="link_modul_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Nomor BA Peminjaman</label>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Nomor BA Peminjaman akan terisi otomatis</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4">
                        <h4 class="text-lg font-medium text-orange-900 dark:text-orange-100 mb-4 flex items-center">
                            ...
                            Informasi Penerimaan BBM
                        </h4>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                <label for="volume_sebelum" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Volume Sisa dari BA Peminjaman (Liter)</label>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Volume sisa dari BA Peminjaman akan terisi otomatis</p>
                            </div>
                            <div>
                                <label for="volume_pengisian" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    Volume Penerimaan (Liter) <span class="text-red-500">*</span>
                                </label>
                            ...
                            <div>
                                <label for="volume_sisa" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Volume Sisa Setelah Penerimaan (Liter)</label>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Dihitung otomatis: Volume Sisa + Volume Penerimaan</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Data Kapal Pemberi (dari BA Peminjaman) -->
                    <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4">
                        <h4 class="text-lg font-medium text-indigo-900 dark:text-indigo-100 mb-4 flex items-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                <label for="kapal_code_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Kode Kapal Pemberi</label>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Kode kapal pemberi akan terisi otomatis</p>
                            </div>
                            <div>
                                <label for="sebab_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Alasan Peminjaman</label>
                                <input type="text" id="sebab_temp" name="sebab_temp" readonly class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-600 dark:text-white cursor-not-allowed">
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Alasan peminjaman dari BA Peminjaman</p>
                            </div>
                        </div>

                        <!-- Nakhoda Kapal Pemberi -->
                        <div class="mt-4">
                            <h5 class="text-md font-medium text-indigo-800 dark:text-indigo-200 mb-3">Nakhoda Kapal Pemberi</h5>
                            <div class="mb-3">
                                <div class="flex items-center">
                                    <input type="checkbox" id="an_nakhoda_temp" name="an_nakhoda_temp" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label for="pangkat_nahkoda_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Pangkat</label>
                        ...
                        <!-- KKM Kapal Pemberi -->
                        <div class="mt-4">
                            <h5 class="text-md font-medium text-indigo-800 dark:text-indigo-200 mb-3">KKM Kapal Pemberi</h5>
                            <div class="mb-3">
                                <div class="flex items-center">
                                    <input type="checkbox" id="an_kkm_temp" name="an_kkm_temp" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
    ...
    <div class="flex items-center justify-center min-h-full py-8">
        <div class="relative mx-auto p-6 border w-11/12 md:w-3/4 lg:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[90vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Detail BA Penerimaan Pinjaman BBM</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Informasi lengkap Berita Acara Penerimaan Pinjaman BBM</p>
                    </div>
...
<div id="helpModal" class="fixed inset-0 bg-black bg-opacity-50 z-[99999] hidden">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Bantuan - BA Penerimaan Pinjaman BBM</h3>
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                <div class="space-y-6">
                    <!-- Overview -->
                    <div>
                        <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Apa itu BA Penerimaan Pinjaman BBM?</h4>
                        <p class="text-gray-600 dark:text-gray-300 mb-4">
                            BA (Berita Acara) Penerimaan Pinjaman BBM adalah dokumen resmi yang mencatat proses penerimaan BBM yang dipinjam dari kapal lain.
                            Dokumen ini melengkapi BA Peminjaman dan mencatat detail penerimaan BBM.
                        </p>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                                ...
                                <ul class="text-sm text-green-800 dark:text-green-200 space-y-1">
                                    <li>• <strong>Pilih Kapal:</strong> Kapal yang menerima BBM pinjaman</li>
                            ...
                            <div class="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg">
                                ...
                                <ul class="text-sm text-purple-800 dark:text-purple-200 space-y-1">
                                    <li>• <strong>Pilih BA Peminjaman:</strong> BA yang terkait dengan peminjaman ini</li>
                            ...
                            <div class="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg">
                                ...
                                <ul class="text-sm text-orange-800 dark:text-orange-200 space-y-1">
                                    <li>• <strong>Jenis BBM:</strong> Tipe bahan bakar yang diterima</li>
                                    <li>• <strong>Volume:</strong> Jumlah BBM yang diterima</li>
                                    <li>• <strong>Kondisi:</strong> Status BBM saat diterima</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-300">Pilih kapal yang menerima BBM</p>
                                </div>
                            </div>
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-300">Pilih BA peminjaman yang terkait</p>
                                </div>
                            </div>
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-300">Periksa dan sesuaikan detail BBM jika perlu</p>
                                </div>
                            </div>
                            <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
                            <ul class="text-sm text-yellow-800 dark:text-yellow-200 space-y-2">
                                <li>• Pastikan BA Peminjaman sudah ada sebelum membuat BA Penerimaan</li>
                                <li>• Data BBM akan terisi otomatis dari BA Peminjaman yang dipilih</li>
                                <li>• Periksa volume BBM sesuai dengan yang dipinjam</li>
                                <li>• BA Penerimaan akan mengubah status BA Peminjaman menjadi "Diterima"</li>
                                <li>• Pastikan data kapal dan BBM akurat sebelum menyimpan</li>
                            </ul>
                        </div>
                    </div>
                </div>
