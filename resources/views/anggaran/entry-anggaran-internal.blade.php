...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Input anggaran internal untuk setiap periode</p>
        </div>
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="anggaranModalLabel" class="text-xl font-semibold text-gray-900 dark:text-white">Form Anggaran Internal</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut untuk input anggaran internal</p>
                    </div>
                ...
                <div class="mt-4">
                    <form id="anggaranForm">
                        ...
                        <div class="mb-6">
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Detail Anggaran Internal per UPT</h4>
...
<script>
    ...
    function showAddForm() {
        ...
        $('#anggaranModalLabel').text('Tambah Anggaran Internal');
    ...
    function saveAnggaran() {
        ...
        let url = isEdit ?
            '{{ route("anggaran.entry-anggaran-internal.update") }}' :
            '{{ route("anggaran.entry-anggaran-internal.create") }}';
    ...
    function viewAnggaran(periode) {
        ...
        $.get('{{ route("anggaran.entry-anggaran-internal.view", [":periode"]) }}'.replace(':periode', periode), function(data) {
    ...
    function editAnggaran(periode) {
        $.get('{{ route("anggaran.entry-anggaran-internal.edit", [":periode"]) }}'.replace(':periode', periode), function(data) {
            ...
            $('#anggaranModalLabel').text('Edit Anggaran Internal');
            $('#anggaranModal').removeClass('hidden');
        });
    }
    ...
    function deleteAnggaran(periode) {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            $.ajax({
                url: '{{ route("anggaran.entry-anggaran-internal.delete", [":periode"]) }}'.replace(':periode', periode)
                , type: 'DELETE'
                , headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
