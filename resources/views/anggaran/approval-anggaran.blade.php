...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Approval Anggaran</h1>
            <p class="text-gray-600 dark:text-gray-400">Setujui atau tolak anggaran yang telah diajukan</p>
        </div>
    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.get('{{ route("anggaran.approval-anggaran.data") }}', {
            search: search
            , status: status
            , date_from: dateFrom
            , date_to: dateTo
    ...
    function viewAnggaran(periode) {
        ...
        $.get('{{ route("anggaran.approval-anggaran.view", [":periode"]) }}'.replace(':periode', periode), function(data) {
    ...
    function approveAnggaran(periode) {
        if (confirm('Apakah Anda yakin ingin menyetujui anggaran periode ' + periode + '?')) {
            $.ajax({
                url: '{{ route("anggaran.approval-anggaran.approve") }}'
                , type: 'POST'
                , data: {
                    periode: periode
                ...
                , success: function(response) {
                    if (response.success) {
                        toastr.success('Anggaran berhasil disetujui');
                        loadData();
                    } else {
