...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Entri Anggaran</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola data anggaran untuk setiap periode</p>
        </div>
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="anggaranModalLabel" class="text-xl font-semibold text-gray-900 dark:text-white">Form Anggaran</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut untuk membuat anggaran baru</p>
                    </div>
    ...
    <div class="flex items-center justify-center min-h-full py-8">
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Detail Anggaran</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lihat detail anggaran per UPT</p>
                    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.get('{{ route("anggaran.entri-anggaran.data") }}', {
            search: search
            , status: status
            , date_from: dateFrom
            , date_to: dateTo
    ...
    function testViewModal() {
        ...
        $('#viewModalBody').html('<div class="text-center py-8">Test modal content</div>');
    }
    ...
    function saveAnggaran() {
        ...
        let url = isEdit ?
            '{{ route("anggaran.entri-anggaran.update") }}' :
            '{{ route("anggaran.entri-anggaran.create") }}';
    ...
    function viewAnggaran(periode, perubahanKe) {
        ...
        $.get('{{ route("anggaran.entri-anggaran.view", [":periode", ":perubahanKe"]) }}'.replace(':periode', periode).replace(':perubahanKe', perubahanKe), function(data) {
    ...
    function editAnggaran(periode, perubahanKe) {
        $.get('{{ route("anggaran.entri-anggaran.edit", [":periode", ":perubahanKe"]) }}'.replace(':periode', periode).replace(':perubahanKe', perubahanKe), function(data) {
    ...
    function deleteAnggaran(periode, perubahanKe) {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            $.ajax({
                url: '{{ route("anggaran.entri-anggaran.delete", [":periode", ":perubahanKe"]) }}'.replace(':periode', periode).replace(':perubahanKe', perubahanKe)
                , type: 'DELETE'
                , headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
