...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Entry Realisasi</h1>
            <p class="text-gray-600 dark:text-gray-400">Input realisasi anggaran untuk setiap periode</p>
        </div>
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="anggaranModalLabel" class="text-xl font-semibold text-gray-900 dark:text-white">Form Entry Realisasi</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut untuk input realisasi anggaran</p>
                    </div>
                ...
                <div class="mt-4">
                    <form id="anggaranForm">
                        ...
                        <div class="mb-6">
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Detail Realisasi per UPT</h4>
                                    ...
                                    <table class="min-w-full border-collapse border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
                                        <thead style="background-color: #568fd2;">
                                            <tr>
                                                ...
                                                <th class="px-4 py-3 text-center text-xs font-medium text-white uppercase tracking-wider border border-gray-300 dark:border-gray-600">Realisasi (Rp)</th>
                                            </tr>
                                        </thead>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mb-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="totalAnggaran" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Total Realisasi</label>
...
<script>
    ...
    function showAddForm() {
        ...
        $('#anggaranModalLabel').text('Tambah Entry Realisasi');
    ...
    function viewAnggaran(periode) {
        ...
        $.get('{{ route("anggaran.entry-realisasi.view", [":periode"]) }}'.replace(':periode', periode), function(data) {
    ...
    function editAnggaran(periode) {
        $.get('{{ route("anggaran.entry-realisasi.edit", [":periode"]) }}'.replace(':periode', periode), function(data) {
    ...
    function deleteAnggaran(periode) {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            $.ajax({
                url: '{{ route("anggaran.entry-realisasi.delete", [":periode"]) }}'.replace(':periode', periode)
                , type: 'DELETE'
                , headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
