...
@section('title', 'Pembatalan Anggaran Internal')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Pembatalan Anggaran Internal</h1>
            <p class="text-gray-600 dark:text-gray-400">Pembatalan anggaran internal yang sudah disetujui</p>
        </div>
    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.get('{{ route("anggaran.pembatalan-anggaran-internal.data") }}', {
            search: search
        }, function(response) {
    ...
    function viewAnggaran(id) {
        ...
        $.get('{{ route("anggaran.pembatalan-anggaran-internal.view", [":id"]) }}'.replace(':id', id), function(data) {
    ...
    function cancelAnggaran(id) {
        if (confirm('Apakah Anda yakin ingin membatalkan anggaran internal ini?')) {
            $.ajax({
                url: '{{ route("anggaran.pembatalan-anggaran-internal.cancel") }}'
                , type: 'POST'
                , data: {
                    id: id
                }
