...
@section('title', 'Laporan Verifikasi Tagihan')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Laporan Verifikasi Tagihan</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Rekapitulasi Verifikasi Realisasi Per UPT</p>
        </div>
    </div>
                ...
                <table class="min-w-full border-collapse border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
                    <thead style="background-color: #568fd2;">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-white uppercase tracking-wider border border-gray-300 dark:border-gray-600">Status Segel</th>
                        </tr>
                    </thead>
...
<script>
    $(document).ready(function() {
        ...
        $('#printBtn').click(function() {
            ...
            form.method = 'POST';
            form.action = '{{ route("laporan-anggaran.export.pdf", "verifikasi-tagihan") }}';
            form.target = '_blank';
        ...
        $('#excelBtn').click(function() {
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("laporan-anggaran.export.excel", "verifikasi-tagihan") }}';
            form.target = '_blank';
        ...
        function loadUptOptions() {
            $.get('{{ route("laporan-anggaran.verifikasi-tagihan.upts") }}', function(data) {
        ...
        function loadNoTagihanOptions() {
            ...
            $.get('{{ route("laporan-anggaran.verifikasi-tagihan.no-tagihan") }}', {
                upt_code: uptCode
            }, function(data) {
        ...
        `);
            ...
            $.get('{{ route("laporan-anggaran.verifikasi-tagihan.data") }}', {
                tgl_awal: tglAwal
                , tgl_akhir: tglAkhir
                , upt_code: uptCode
                ...
                if (data.data && data.data.length > 0) {
                    ...
                    data.data.forEach(function(item, index) {
                        const statusSegel = item.status_segel == 1 ? 'BAIK' : 'RUSAK';
                        const statusClass = item.status_segel == 1 ? 'text-green-600' : 'text-red-600';
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-4 py-3 text-center border border-gray-300 dark:border-gray-600">
                                <span class="px-2 py-1 text-xs font-medium rounded-full ${statusClass}">${statusSegel}</span>
                            </td>
                        </tr>
                    `;
                    });
