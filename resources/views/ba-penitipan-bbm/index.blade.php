...
@section('title', 'BA Penitipan Bbm')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">BA Penitipan Bbm</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola Berita Acara Penitipan Bbm</p>
        </div>
   ...
   `);
        ...
        function setDefaultDates() {
            ...
            try {
                ...
                // Field tanggal_sebelum dan tanggal_pengisian sudah tidak ada di BA Penitipan BBM
        ...
        function resetForm() {
            ...
            try {
                ...
                if ($('#modalTitle').length > 0) {
                    $('#modalTitle').text('Form Tambah BA Penitipan BBM');
                }
        ...
        function loadData() {
            ...
            $.ajax({
                url: '{{ route("ba-penitipan-bbm.data") }}'
                , type: 'GET'
                , data: params
                , beforeSend: function() {
                    showLoading();
        ...
        function loadKapalData(kapalId) {
            $.ajax({
                url: '{{ route("ba-penitipan-bbm.kapal-data") }}'
                , type: 'GET'
                , data: {
                    kapal_id: kapalId
                }
        ...
        function loadBaData(kapalId) {
            ...
            $.ajax({
                url: '{{ route("ba-penitipan-bbm.ba-data") }}'
                , type: 'GET'
                , data: {
                    kapal_id: kapalId
       ...
       `;
        ...
        function fillEditForm(data) {
            ...
            $('#baForm')[0].reset();
            $('#modalTitle').text('Form Edit BA Penitipan BBM');
        ...
        function setupEventHandlers() {
            ...
            $('#deleteDocumentBtn').on('click', function() {
                if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                    $.ajax({
                        url: `/ba-penitipan-bbm/${currentBaId}/delete-document`
                        , type: 'DELETE'
                        , data: {
                            _token: '{{ csrf_token() }}'
                        }
        ...
        function handleFormSubmit(e) {
            ...
            let url = '{{ route("ba-penitipan-bbm.store") }}';
            let method = 'POST';

            if (currentEditMode) {
                url = `{{ url('ba-penitipan-bbm') }}/${currentBaId}`;
        ...
        function handleUploadSubmit(e) {
            ...
            $.ajax({
                url: `{{ url('ba-penitipan-bbm') }}/${currentBaId}/upload`
                , type: 'POST'
                , data: formData
                , processData: false
                , contentType: false
        ...
        function handleViewDocument() {
            ...
            $.ajax({
                url: `{{ url('ba-penitipan-bbm') }}/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
        ...
        function deleteDocument(baId) {
            ...
            $.ajax({
                url: `{{ url('ba-penitipan-bbm') }}/${baId}/delete-document`
            ...
            $.ajax({
                ...
                , success: function(response) {
                    if (response.success) {
                        ...
                        currentBaId = id;
                        currentEditMode = true;
                        $('#modalTitle').text('Edit BA Penitipan BBM');
                        $('#submitBtn').html('<i class="fas fa-save me-2"></i>Update BA');
                        $('#baModal').removeClass('hidden');
                    } else {
        ...
        window.viewDocument = function(baId) {
            ...
            $.ajax({
                url: `/ba-penitipan-bbm/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        $('#documentViewer').html(`
    ...
    <div class="relative mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 mt-10 mb-10 max-h-[90vh] overflow-y-auto help-modal-scroll">
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between pb-4">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">Panduan BA Penitipan BBM</h3>
            ...
            <div class="space-y-6">
                <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                    <h4 class="text-lg font-semibold text-blue-900 dark:text-blue-100 mb-3">Tentang BA Penitipan BBM</h4>
                    <p class="text-blue-800 dark:text-blue-200 text-sm leading-relaxed">
                        Berita Acara Penitipan BBM digunakan untuk mencatat penitipan BBM dari kapal ke penyedia.
                        Dokumen ini berisi informasi tentang volume BBM yang dititipkan, penyedia, dan kondisi penitipan.
                    </p>
                </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Pilih kapal yang akan menitipkan BBM. Data kapal akan otomatis terisi.</p>
                            </div>
                        </div>
