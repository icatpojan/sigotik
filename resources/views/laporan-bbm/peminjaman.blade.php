...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Laporan peminjaman BBM untuk periode tertentu</p>
        </div>
    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.ajax({
            url: '{{ route("laporan-bbm.peminjaman.data") }}'
            , method: 'GET'
            , data: {
    ...
    function exportData(format = 'excel') {
        ...
        fetch(`{{ route("laporan-bbm.peminjaman.export") }}?${params}`)
            .then(response => {
                if (response.ok) {
            ...
            .then(blob => {
                ...
                a.href = url;
                a.download = `laporan_peminjaman_${format}_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
