...
@section('title', 'LAP Detail Penggunaan & Penerimaan BBM')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">LAP Detail Penggunaan & Penerimaan BBM</h1>
            <p class="text-gray-600 dark:text-gray-400">Laporan detail penggunaan dan penerimaan BBM</p>
        </div>
    </div>
            ...
            <table class="min-w-full border-collapse border border-gray-300 dark:border-gray-600">
                <thead style="background-color: #568fd2;">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider border border-gray-300 dark:border-gray-600">Tanggal BA</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider border border-gray-300 dark:border-gray-600">Nomor BA</th>
...
<script>
    ...
    function loadKapalOptions(uptId) {
        console.log('Loading kapal options for UPT:', uptId);
        $.get('/laporan-bbm/kapal-options', {
                upt_id: uptId
            })
            .done(function(data) {
                console.log('Kapal options loaded:', data);
            ...
            .fail(function(xhr, status, error) {
                console.error('Error loading kapal options:', error);
                console.error('Response:', xhr.responseText);
                const select = $('#kapal_id');
                select.empty().append('<option value="">Error loading kapal data</option>');
            });
    }
    ...
    function loadData() {
        ...
        $.ajax({
            url: '{{ route("laporan-bbm.detail-penggunaan-penerimaan.data") }}'
            , method: 'GET'
            , data: {
            ...
            , success: function(response) {
                ...
                if (response.data.length > 0) {
                    response.data.forEach(function(item, index) {
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 text-center border border-gray-300 dark:border-gray-600">
                                <div class="text-sm text-gray-900 dark:text-white">${formatNumber(item.penerimaan || 0)} Liter</div>
                            </td>
                            <td class="px-6 py-4 text-center border border-gray-300 dark:border-gray-600">
                                <div class="text-sm text-gray-900 dark:text-white">${formatNumber(item.penggunaan || 0)} Liter</div>
                            </td>
                        </tr>
                    `;
                        tbody.append(row);
                    ...
                    <tr class="bg-gray-100 dark:bg-gray-700 font-semibold">
                        <td colspan="4" class="px-6 py-4 text-center border border-gray-300 dark:border-gray-600">
    ...
    function exportData(format = 'excel') {
        ...
        fetch(`{{ route("laporan-bbm.detail-penggunaan-penerimaan.export") }}?${params}`)
            .then(response => {
                if (response.ok) {
