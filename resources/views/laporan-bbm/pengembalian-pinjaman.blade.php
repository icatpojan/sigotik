...
@section('title', 'Laporan Pengembalian Pinjaman')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Laporan Pengembalian Pinjaman</h1>
            <p class="text-gray-600 dark:text-gray-400">Laporan pengembalian pinjaman BBM untuk periode tertentu</p>
        </div>
    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.ajax({
            url: '{{ route("laporan-bbm.pengembalian-pinjaman.data") }}'
            , method: 'GET'
            , data: {
    ...
    function exportData(format = 'excel') {
        ...
        fetch(`{{ route("laporan-bbm.pengembalian-pinjaman.export") }}?${params}`)
            .then(response => {
                if (response.ok) {
            ...
            .then(blob => {
                ...
                a.href = url;
                a.download = `laporan_pengembalian_pinjaman_${format}_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
