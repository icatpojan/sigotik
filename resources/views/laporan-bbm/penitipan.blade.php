...
@section('title', 'Laporan Penitipan BBM')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Laporan Penitipan BBM</h1>
            <p class="text-gray-600 dark:text-gray-400">Laporan penitipan BBM untuk periode tertentu</p>
        </div>
    </div>
...
<script>
    ...
    function loadData() {
        ...
        $.ajax({
            url: '{{ route("laporan-bbm.penitipan.data") }}'
            , method: 'GET'
            , data: {
    ...
    function exportData(format = 'excel') {
        ...
        fetch(`{{ route("laporan-bbm.penitipan.export") }}?${params}`)
            .then(response => {
                if (response.ok) {
            ...
            .then(blob => {
                ...
                a.href = url;
                a.download = `laporan_penitipan_${format}_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
