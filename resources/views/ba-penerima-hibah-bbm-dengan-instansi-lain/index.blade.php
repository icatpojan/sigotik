...
@section('title', 'BA Penerima Hibah BBM Dengan Instansi Lain')

@section('content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">BA Penerima Hibah BBM Dengan Instansi Lain</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola Berita Acara Penerima Hibah BBM Dengan Instansi Lain</p>
        </div>
            ...
            <table class="min-w-full border-collapse border border-gray-300 dark:border-gray-600">
                <thead style="background-color: #568fd2;">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-white uppercase tracking-wider border border-gray-300 dark:border-gray-600">Instansi Pemberi</th>
...
<script>
    $(document).ready(function() {
        ...
        function showLoading() {
            $('#baTableBody').html('<tr><td colspan="5" class="px-6 py-4 text-center"><div class="flex items-center justify-center"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div><span class="ml-2">Memuat data...</span></div></td></tr>');
        }
        ...
        function showError(msg) {
            // Check if message is "Unauthenticated" and refresh page
            if (msg === "Unauthenticated.") {
                if (typeof toastr !== 'undefined') {
                    toastr.warning('Sesi Anda telah berakhir. Halaman akan dimuat ulang.');
                }
                setTimeout(() => {
        ...
        function resetForm() {
            $('#baForm')[0].reset();
            $('#modalTitle').text('Form Tambah BA Penerima Hibah BBM Dengan Instansi Lain');
            $('#submitBtn').html('Simpan BA');
            currentBaId = null;
            currentEditMode = false;
            clearKapalData();
            setDefaultDates();
            $('#an_staf, #an_nakhoda, #an_kkm, #an_nakhoda_temp, #an_kkm_temp').prop('checked', false);
        ...
        function loadData() {
            $.ajax({
                url: '{{ route("ba-penerima-hibah-bbm-dengan-instansi-lain.data") }}'
                , type: 'GET'
                , data: {
                    page: currentPage
                    , per_page: perPage
        ...
        function loadKapalData(kapalId) {
            $.ajax({
                url: '{{ route("ba-penerima-hibah-bbm-dengan-instansi-lain.kapal-data") }}'
                , type: 'GET'
                , data: {
                    kapal_id: kapalId
                }
                , success: function(response) {
                    if (response.success) {
                        ...
                        // Only fill these fields if not in edit mode
                        if (!currentEditMode) {
                            $('#nama_nahkoda').val(d.nama_nakoda);
                            $('#nip_nahkoda').val(d.nip_nakoda);
                            $('#pangkat_nahkoda').val(d.pangkat_nahkoda);
                            $('#nama_kkm').val(d.nama_kkm);
                            $('#nip_kkm').val(d.nip_kkm);
                            $('#pangkat_kkm').val(d.pangkat_kkm);
        ...
        function loadBaSebelumPengisianData() {
            const tanggal = $('#tanggal_surat').val();
            const kapalId = $('#kapal_id').val();

            if (tanggal && kapalId) {
                $.ajax({
                    url: '{{ route("ba-penerima-hibah-bbm-dengan-instansi-lain.ba-sebelum-pengisian-data") }}'
                    , type: 'GET'
                    , data: {
                    ...
                    , success: function(response) {
                        if (response.success) {
                            const d = response.data;
                            $('#link_ba').val(d.link_ba);
                            $('#volume_sebelum').val(d.volume_sebelum);
                            calculateVolumeSisa();
                        } else {
                            $('#link_ba').val('');
                            $('#volume_sebelum').val('');
                ...
                // Clear fields jika tidak ada tanggal atau kapal
                $('#link_ba').val('');
                $('#volume_sebelum').val('');
                $('#volume_sisa').val('');
            }
        ...
        function loadPersetujuanData(persetujuanId) {
            $.ajax({
                url: '{{ route("ba-penerima-hibah-bbm-dengan-instansi-lain.persetujuan-data") }}'
                , type: 'GET'
                , data: {
                ...
                , success: function(response) {
                    if (response.success) {
                        $('#nomor_persetujuan').val(response.data.nomor_persetujuan);
                        $('#tanggal_persetujuan').val(formatDateForInput(response.data.tanggal_persetujuan));
                    }
                }
            });
        }
        ...
        function clearKapalData() {
            $('#code_kapal, #alamat_upt, #nama_kapal_pemberi, #sebab_temp, #nama_staf_pangkalan, #nip_staf, #jabatan_staf_pangkalan, #nama_nahkoda, #nip_nahkoda, #pangkat_nahkoda, #nama_kkm, #nip_kkm, #pangkat_kkm, #nama_nahkoda_temp, #pangkat_nahkoda_temp, #nip_nahkoda_temp, #nama_kkm_temp, #nip_kkm_temp, #link_ba, #volume_sebelum, #volume_sisa').val('');
        }
        ...
        function renderPagination(pagination) {
            ...
            let html = '<div class="flex items-center justify-between mt-4"><div class="text-sm">Menampilkan ' + (pagination.from || 0) + ' sampai ' + (pagination.to || 0) + ' dari ' + pagination.total + ' data</div><div class="flex space-x-1">';

            if (pagination.current_page > 1) {
                html += `<button onclick="changePage(${pagination.current_page - 1})" class="px-3 py-1 text-sm bg-white border rounded hover:bg-gray-50">Previous</button>`;
            }
            ...
            if (pagination.current_page < pagination.last_page) {
                html += `<button onclick="changePage(${pagination.current_page + 1})" class="px-3 py-1 text-sm bg-white border rounded hover:bg-gray-50">Next</button>`;
            }
        ...
        function setupEventHandlers() {
            ...
            $('#deleteDocumentBtn').on('click', function() {
                if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                    $.ajax({
                        url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${currentBaId}/delete-document`
                        , type: 'DELETE'
                        , data: {
                            _token: '{{ csrf_token() }}'
                        }
            ...
            $('#kapal_id').on('change', function() {
                ...
                if (kapalId) {
                    ...
                    // Jika tanggal sudah ada dan tidak dalam mode edit, langsung ambil data BA Sebelum Pengisian
                    if ($('#tanggal_surat').val() && !currentEditMode) {
                        loadBaSebelumPengisianData();
                    }
                } else {
                    clearKapalData();
                }
            ...
            $('#tanggal_surat').on('change', function() {
                // Only load BA Sebelum Pengisian data if not in edit mode
                if (!currentEditMode) {
                    loadBaSebelumPengisianData();
                }
            });
        ...
        function saveBa() {
            ...
            const url = currentEditMode ? `/ba-penerima-hibah-bbm-dengan-instansi-lain/${currentBaId}` : '{{ route("ba-penerima-hibah-bbm-dengan-instansi-lain.store") }}';
        ...
        window.viewBa = function(id) {
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${id}`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        const ba = response.data;
                        let html = '<div class="grid grid-cols-2 gap-4">';
                        html += `<div><strong>Nomor:</strong> ${ba.nomor_surat}</div>`;
                        html += `<div><strong>Tanggal:</strong> ${formatDate(ba.tanggal_surat)}</div>`;
                        html += `<div><strong>Kapal Penerima:</strong> ${ba.kapal ? ba.kapal.nama_kapal : ba.kapal_code}</div>`;
                        html += `<div><strong>Nama Kapal Pemberi:</strong> ${ba.kapal_code_temp}</div>`;
                        html += `<div><strong>Volume:</strong> ${formatNumber(ba.volume_pengisian)} L</div>`;
                        html += `<div><strong>BBM:</strong> ${ba.keterangan_jenis_bbm}</div>`;
                        html += '</div>';
        ...
        window.editBa = function(id) {
            currentBaId = id;
            currentEditMode = true; // Set edit mode first
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${id}`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        // Fill form with BA data - no need to load kapal data in edit mode
        ...
        function fillEditForm(ba) {
            ...
            $('#nama_kapal_pemberi').val(ba.kapal_code_temp);
            ...
            $('#nama_staf_pangkalan').val(ba.nama_staf_pagkalan || ba.nama_staf_pangkalan);
            ...
            $('#pangkat_kkm').val(ba.pangkat_kkm);
            $('#nama_nahkoda_temp').val(ba.nama_nahkoda_temp);
            $('#pangkat_nahkoda_temp').val(ba.pangkat_nahkoda_temp);
            $('#nip_nahkoda_temp').val(ba.nip_nahkoda_temp);
            $('#nama_kkm_temp').val(ba.nama_kkm_temp);
            $('#nip_kkm_temp').val(ba.nip_kkm_temp);
            $('#an_staf').prop('checked', ba.an_staf == 1);
            $('#an_nakhoda').prop('checked', ba.an_nakhoda == 1);
            $('#an_kkm').prop('checked', ba.an_kkm == 1);
            $('#an_nakhoda_temp').prop('checked', ba.an_nakhoda_temp == 1);
            $('#an_kkm_temp').prop('checked', ba.an_kkm_temp == 1);
        }

        window.deleteBa = function(id) {
            if (!confirm('Yakin hapus BA ini?')) return;
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${id}`
                , type: 'POST'
                , data: {
        ...
        window.generatePdf = function(baId) {
            ...
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${baId}/pdf`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
        ...
        function uploadDocument() {
            ...
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${currentBaId}/upload`
                , type: 'POST'
                , data: formData
                , processData: false
                , contentType: false
        ...
        window.viewDocument = function(baId) {
            $.ajax({
                url: `/ba-penerima-hibah-bbm-dengan-instansi-lain/${baId}/view-document`
                , type: 'GET'
                , success: function(response) {
                    if (response.success) {
                        $('#documentViewer').html(`<div class="text-center"><h4 class="text-lg mb-4">${response.filename}</h4><iframe src="${response.file_url}" width="100%" height="600px" class="border rounded-lg"></iframe><div class="mt-4"><a href="${response.file_url}" target="_blank" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg inline-flex items-center">Buka Tab Baru</a></div></div>`);
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="modalTitle" class="text-xl font-semibold text-gray-900 dark:text-white">Form Tambah BA Penerima Hibah BBM Dengan Instansi Lain</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut</p>
                    </div>
                ...
                <form id="baForm" class="mt-6 space-y-6">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="kapal_id" name="kapal_id" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                            ...
                            <div>
                                ...
                                <input type="text" id="code_kapal" readonly class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-100 dark:bg-gray-600 cursor-not-allowed">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <textarea id="alamat_upt" rows="3" readonly class="w-full px-4 py-3 border rounded-lg bg-gray-100 dark:bg-gray-600 cursor-not-allowed resize-none"></textarea>
                            </div>
                            <div>
                                ...
                                <textarea id="lokasi_surat" name="lokasi_surat" rows="3" required placeholder="Lokasi..." class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white resize-none"></textarea>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="nomor_surat" name="nomor_surat" required placeholder="BA/001/KKP/2024" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="tanggal_surat" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    Tanggal <span class="text-red-500">*</span>
                                </label>
                                <input type="date" id="tanggal_surat" name="tanggal_surat" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                ...
                                <input type="time" id="jam_surat" name="jam_surat" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                        <div class="mt-4">
                            <label for="zona_waktu_surat" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Zona Waktu <span class="text-red-500">*</span></label>
                            <select id="zona_waktu_surat" name="zona_waktu_surat" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="nama_kapal_pemberi" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    Nama Kapal Pemberi Hibah <span class="text-red-500">*</span>
                                </label>
                                <input type="text" id="nama_kapal_pemberi" name="nama_kapal_pemberi" required placeholder="Nama kapal pemberi" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="link_ba" name="link_ba" readonly class="w-full px-4 py-3 border rounded-lg bg-gray-100 dark:bg-gray-600 cursor-not-allowed">
                            </div>
                            <div>
                                <label for="volume_sebelum" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">BBM Sebelum Pengisian (Liter)</label>
                                <input type="number" id="volume_sebelum" name="volume_sebelum" step="0.01" min="0" readonly class="w-full px-4 py-3 border rounded-lg bg-gray-100 dark:bg-gray-600 cursor-not-allowed">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" id="keterangan_jenis_bbm" name="keterangan_jenis_bbm" required placeholder="Jenis BBM" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="volume_pengisian" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Jumlah BBM Diterima (Liter) <span class="text-red-500">*</span></label>
                                <input type="number" id="volume_pengisian" name="volume_pengisian" step="0.01" min="0" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="volume_sisa" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Sisa BBM (Liter)</label>
                                <input type="number" id="volume_sisa" name="volume_sisa" step="0.01" min="0" readonly class="w-full px-4 py-3 border rounded-lg bg-gray-100 dark:bg-gray-600 cursor-not-allowed">
                            </div>
                        </div>
                        <div class="mt-4">
                            <label for="sebab_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Alasan Hibah BBM <span class="text-red-500">*</span></label>
                            <textarea id="sebab_temp" name="sebab_temp" required placeholder="Masukkan alasan hibah BBM" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4">
                        <h4 class="text-lg font-medium text-indigo-900 dark:text-indigo-100 mb-4 flex items-center">
                            ...
                            PEJABAT/STAF UPT PENERIMA HIBAH
                        </h4>
                        <div class="mb-4">
                            <div class="flex items-center">
                                ...
                                <label for="an_staf" class="ml-2 block text-sm font-medium text-gray-700 dark:text-gray-300">An. Staf Pangkalan</label>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div>
                    ...
                    <div class="bg-cyan-50 dark:bg-cyan-900/20 rounded-lg p-4">
                        <h4 class="text-lg font-medium text-cyan-900 dark:text-cyan-100 mb-4 flex items-center">
                            ...
                            NAKHODA KAPAL PENERIMA HIBAH
                        </h4>
                        <div class="mb-3">
                            <input type="checkbox" id="an_nakhoda" name="an_nakhoda" value="1" class="h-4 w-4">
                            <label for="an_nakhoda" class="ml-2 text-sm font-medium">An. Nakhoda</label>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" id="nama_nahkoda" name="nama_nahkoda" placeholder="Nama nakhoda" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="pangkat_nahkoda" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Pangkat/Gol</label>
                                <input type="text" id="pangkat_nahkoda" name="pangkat_nahkoda" placeholder="Pangkat/Gol" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="nip_nahkoda" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NIP</label>
                                <input type="text" id="nip_nahkoda" name="nip_nahkoda" placeholder="NIP" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                        <h4 class="text-lg font-medium text-blue-900 dark:text-blue-100 mb-4 flex items-center">
                            ...
                            KKM KAPAL PENERIMA HIBAH
                        </h4>
                        <div class="mb-3">
                            <input type="checkbox" id="an_kkm" name="an_kkm" value="1" class="h-4 w-4">
                            <label for="an_kkm" class="ml-2 text-sm font-medium">An. KKM</label>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div>
                                <label for="nama_kkm" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NAMA KKM</label>
                                <input type="text" id="nama_kkm" name="nama_kkm" placeholder="Nama KKM" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="pangkat_kkm" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Pangkat/Gol</label>
                                <input type="text" id="pangkat_kkm" name="pangkat_kkm" placeholder="Pangkat/Gol" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            <div>
                                <label for="nip_kkm" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NIP</label>
                                <input type="text" id="nip_kkm" name="nip_kkm" placeholder="NIP" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                        ...
                        <div class="mb-3">
                            <input type="checkbox" id="an_nakhoda_temp" name="an_nakhoda_temp" value="1" class="h-4 w-4">
                            <label for="an_nakhoda_temp" class="ml-2 text-sm font-medium">An. Nakhoda Pemberi</label>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                            <div>
                                <label for="nama_nahkoda_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NAMA NAKHODA Pemberi Hibah</label>
                                <input type="text" id="nama_nahkoda_temp" name="nama_nahkoda_temp" placeholder="Nama nakhoda pemberi" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
                            </div>
                            <div>
                                <label for="pangkat_nahkoda_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Pangkat/Gol</label>
                                <input type="text" id="pangkat_nahkoda_temp" name="pangkat_nahkoda_temp" placeholder="Pangkat/Gol" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
                            </div>
                            <div>
                                <label for="nip_nahkoda_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NIP</label>
                                <input type="text" id="nip_nahkoda_temp" name="nip_nahkoda_temp" placeholder="NIP" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
                            </div>
                        </div>
                        <div class="mb-3">
                            <input type="checkbox" id="an_kkm_temp" name="an_kkm_temp" value="1" class="h-4 w-4">
                            <label for="an_kkm_temp" class="ml-2 text-sm font-medium">An. KKM Pemberi</label>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div>
                                <label for="nama_kkm_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NAMA KKM Pemberi Hibah</label>
                                <input type="text" id="nama_kkm_temp" name="nama_kkm_temp" placeholder="Nama KKM pemberi" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
                            </div>
                            <div>
                                <label for="nip_kkm_temp" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">NIP</label>
                                <input type="text" id="nip_kkm_temp" name="nip_kkm_temp" placeholder="NIP" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white transition-colors">
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <button type="button" id="cancelBtn" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-lg">Batal</button>
                    <button type="submit" form="baForm" id="submitBtn" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg">Simpan BA</button>
                </div>
            </div>
        </div>
    </div>
...
<div id="viewDocumentModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[99999] hidden">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden">
        ...
        <div class="flex justify-end space-x-3 p-6 border-t">
            <button id="deleteDocumentBtn" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg">Hapus Dokumen</button>
            <button id="closeViewDocumentModal" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg">Tutup</button>
        </div>
    </div>
</div>
    ...
    <div class="relative mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 mt-10 mb-10 max-h-[90vh] overflow-y-auto help-modal-scroll">
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between pb-4">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">Panduan BA Penerima Hibah BBM Dengan Instansi Lain</h3>
            ...
            <div class="space-y-6">
                <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                    <h4 class="text-lg font-semibold text-blue-900 dark:text-blue-100 mb-3">Tentang BA Penerima Hibah BBM Dengan Instansi Lain</h4>
                    <p class="text-blue-800 dark:text-blue-200 text-sm leading-relaxed">
                        Berita Acara Penerima Hibah BBM Dengan Instansi Lain digunakan untuk mencatat penerimaan hibah BBM dari instansi lain di luar kapal pengawas.
                        Dokumen ini berisi informasi tentang instansi penerima, instansi pemberi, volume BBM yang diterima, dan kondisi penerimaan.
                    </p>
                </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Isi informasi instansi yang menerima hibah BBM, termasuk nama instansi dan alamat.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Isi informasi instansi pemberi hibah BBM, termasuk nama instansi dan alamat.</p>
                            </div>
                        </div>
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                    ...
                    <ul class="text-yellow-800 dark:text-yellow-200 text-sm space-y-1">
                        <li>• Pastikan data instansi penerima dan pemberi akurat</li>
                        ...
                        <li>• Pastikan koordinasi dengan instansi pemberi telah dilakukan</li>
                    </ul>
                </div>
            </div>
        </div>
