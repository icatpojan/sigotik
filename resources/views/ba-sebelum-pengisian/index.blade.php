...
@section('title', 'BA Sebelum Pengisian')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">BA Sebelum Pengisian</h1>
            <p class="text-gray-600 dark:text-gray-400">Kelola Berita Acara Sebelum Pengisian</p>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#createBaBtn, #createFirstBaBtn').click(function() {
            $('#modalTitle').text('Form Tambah BA Sebelum Pengisian');
            $('#baForm')[0].reset();
            $('#baId').val('');
        ...
        $('#kapal_id').change(function() {
            ...
            if (kapalId) {
                ...
                $.ajax({
                    url: '/ba-sebelum-pengisian/kapal-data'
                    , type: 'GET'
                    , data: {
                        kapal_id: kapalId
                    }
        ...
        $('#baForm').on('submit', function(e) {
            ...
            const baId = $('#baId').val();
            const url = baId ? `/ba-sebelum-pengisian/${baId}` : '/ba-sebelum-pengisian';
    ...
    function editBa(baId) {
        $.ajax({
            ...
            , success: function(response) {
                if (response.success) {
                    const ba = response.data;
                    $('#modalTitle').text('Form Edit BA Sebelum Pengisian');
                    ...
                    // Load data kapal hanya untuk field yang kosong atau untuk alamat UPT
                    if (ba.kapal) {
                        $.ajax({
                            url: '/ba-sebelum-pengisian/kapal-data'
                            , type: 'GET'
                            , data: {
                                kapal_id: ba.kapal.m_kapal_id
                            }
                            , success: function(kapalResponse) {
                                if (kapalResponse.success) {
                                    const kapalData = kapalResponse.data;
                                    // Hanya isi alamat UPT dari data kapal, jangan timpa data BA
                                    $('#alamat_upt').val(kapalData.alamat_upt);

                                    // Hanya isi field yang kosong dari data BA
                                    if (!$('#zona_waktu_surat').val()) {
                                        $('#zona_waktu_surat').val(kapalData.zona_waktu_upt);
                                    }
                                    if (!$('#lokasi_surat').val()) {
                                        $('#lokasi_surat').val(kapalData.kota);
                                    }
                                    if (!$('#jabatan_staf_pangkalan').val()) {
                                        $('#jabatan_staf_pangkalan').val(kapalData.jabatan_petugas);
                                    }
                                    if (!$('#nama_staf_pagkalan').val()) {
                                        $('#nama_staf_pagkalan').val(kapalData.nama_petugas);
                                    }
                                    if (!$('#nip_staf').val()) {
                                        $('#nip_staf').val(kapalData.nip_petugas);
                                    }
                                    if (!$('#nama_nahkoda').val()) {
                                        $('#nama_nahkoda').val(kapalData.nama_nakoda);
                                    }
                                    if (!$('#nip_nahkoda').val()) {
                                        $('#nip_nahkoda').val(kapalData.nip_nakoda);
                                    }
                                    if (!$('#nama_kkm').val()) {
                                        $('#nama_kkm').val(kapalData.nama_kkm);
                                    }
                                    if (!$('#nip_kkm').val()) {
                                        $('#nip_kkm').val(kapalData.nip_kkm);
                                    }
                                }
                            }
                            , error: function() {
                                console.log('Gagal memuat data kapal untuk edit');
                            }
                        });
                    }
    ...
    function generatePdf(baId) {
        ...
        $.ajax({
            url: `/ba-sebelum-pengisian/${baId}/pdf`
            , type: 'GET'
    ...
    function viewDocument(baId) {
        ...
        $.ajax({
            url: `/ba-sebelum-pengisian/${baId}/view-document`
            , type: 'GET'
            , success: function(response) {
                if (response.success) {
                    $('#documentViewer').html(`
                        <div class="text-center">
                            <div class="mb-4">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">Dokumen pendukung BA Sebelum Pengisian</p>
                            </div>
    ...
    $(document).ready(function() {
        $('#uploadForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `/ba-sebelum-pengisian/${currentBaId}/upload`
                , type: 'POST'
                , data: formData
                , processData: false
                , contentType: false
        ...
        $('#deleteDocumentBtn').on('click', function() {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                ...
                $.ajax({
                    url: `/ba-sebelum-pengisian/${currentBaId}/delete-document`
                    , type: 'DELETE'
                    , data: {
                        _token: $('meta[name="csrf-token"]').attr('content')
                    }
        ...
        <div class="relative mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[95vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 id="modalTitle" class="text-xl font-semibold text-gray-900 dark:text-white">Form Tambah BA Sebelum Pengisian</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Lengkapi data berikut untuk membuat Berita Acara Sebelum Pengisian</p>
                    </div>
                    ...
                    <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                        ...
                        <div>
                            <label for="volume_sisa" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                Sisa BBM Sebelum Pengisian (Liter) <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
    ...
    <div class="flex items-center justify-center min-h-full py-8">
        <div class="relative mx-auto p-6 border w-11/12 md:w-3/4 lg:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 max-h-[90vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex items-center justify-between pb-6 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Detail BA Sebelum Pengisian</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Informasi lengkap Berita Acara Sebelum Pengisian</p>
                    </div>
    ...
    <div class="relative mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-lg bg-white dark:bg-gray-800 mt-10 mb-10 max-h-[90vh] overflow-y-auto help-modal-scroll">
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between pb-4">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">Panduan BA Sebelum Pengisian</h3>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-lg">
                    ...
                    <p class="text-blue-800 dark:text-blue-200">
                        BA Sebelum Pengisian adalah dokumen yang dibuat sebelum proses pengisian BBM kapal.
                        Dokumen ini mencatat kondisi awal tangki BBM sebelum pengisian dilakukan.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-300">
                                Klik tombol "Tambah BA" untuk membuat BA Sebelum Pengisian baru.
                            </p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                ...
                <!-- Form Fields -->
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">📝 Field Form BA Sebelum Pengisian</h4>
                    <div class="space-y-3">
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            ...
                            <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                                <li>• <strong>Kapal:</strong> Pilih kapal yang akan diisi BBM</li>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            ...
                            <ul class="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                                <li>• <strong>Volume Sebelum:</strong> Volume BBM di tangki sebelum pengisian</li>
                                <li>• <strong>Jenis BBM:</strong> Jenis bahan bakar yang akan diisi</li>
                                <li>• <strong>Kondisi Tangki:</strong> Kondisi tangki sebelum pengisian</li>
                            </ul>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
