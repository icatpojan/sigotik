...
class BaSebelumPelayaranController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('ba-sebelum-pelayaran.index', compact('kapals'));
    }

    /**
     * Get BA Sebelum Pelayaran data via AJAX
     */
    public function getData(Request $request)
    {
        $query = BbmKapaltrans::with(['kapal'])
            ->where('status_ba', 6); // BA Sebelum Pelayaran
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'nama_nahkoda' => 'nullable|string|max:30', // Sesuai migration
        ...
        try {
            ...
            $ba = BbmKapaltrans::create([
                ...
                'status_ba' => 6, // BA Sebelum Pelayaran
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Sebelum Pelayaran berhasil dibuat',
                'data' => $ba->load('kapal')
            ]);
        } catch (\Exception $e) {
            return response()->json([
     ...
     */
    public function show(BbmKapaltrans $baSebelumPelayaran)
    {
        return response()->json([
            'success' => true,
            'data' => $baSebelumPelayaran->load('kapal')
        ]);
    }
     ...
     */
    public function update(Request $request, BbmKapaltrans $baSebelumPelayaran)
    {
        $validator = Validator::make($request->all(), [
            'kapal_id' => 'required|exists:m_kapal,m_kapal_id',
            'nomor_surat' => 'required|string|max:50|unique:bbm_kapaltrans,nomor_surat,' . $baSebelumPelayaran->trans_id . ',trans_id',
            'tanggal_surat' => 'required|date',
            'jam_surat' => 'required|date_format:H:i',
            'zona_waktu_surat' => 'required|in:WIB,WITA,WIT',
            'lokasi_surat' => 'required|string', // TEXT field, no max limit
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Sebelum Pelayaran berhasil diperbarui',
                'data' => $baSebelumPelayaran->load('kapal')
            ]);
        } catch (\Exception $e) {
            return response()->json([
     ...
     */
    public function destroy(BbmKapaltrans $baSebelumPelayaran)
    {
        try {
            $baSebelumPelayaran->delete();

            return response()->json([
                'success' => true,
                'message' => 'BA Sebelum Pelayaran berhasil dihapus'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
     ...
     */
    public function generatePdf(BbmKapaltrans $baSebelumPelayaran)
    {
        try {
            // Load relationship data
            $baSebelumPelayaran->load(['kapal.upt']);

            // Get data
            $data = $baSebelumPelayaran;
            $kapal = $data->kapal;
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="center">
                            <font size="12"><b><u>BERITA ACARA SISA BBM SEBELUM PELAYARAN</b></u></font>
                        </td>
                    </tr>
                    <tr>
                        <td width="100%" align="center">
                    ...
                    <tr>
                        <td width="100%" align="justify">Menyatakan bahwa telah melakukan pengukuran sisa BBM sebelum pelayaran dengan rincian sebagai berikut :
                        </td>
                    </tr>
                </table>';
            ...
            $html .= '<br><br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="justify">Demikian Berita Acara Sisa BBM Sebelum Pelayaran ini dibuat dengan sebenar – benarnya untuk dapat dipergunakan sebagaimana mestinya.
                        </td>
                    </tr>
                </table>';
            ...
            // Generate filename
            $filename = 'BA_Sebelum_Pelayaran_' . str_replace('/', '_', $data->nomor_surat) . '_' . date('Y-m-d_H-i-s');
    ...
    public function uploadDocument(Request $request, $id)
    {
        ...
        try {
            $ba = BbmKapaltrans::where('status_ba', 6)->findOrFail($id);
