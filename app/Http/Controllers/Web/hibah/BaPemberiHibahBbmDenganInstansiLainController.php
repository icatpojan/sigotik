...
class BaPemberiHibahBbmDenganInstansiLainController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('ba-pemberi-hibah-bbm-dengan-instansi-lain.index', compact('kapals', 'persetujuans'));
    }

    /**
     * Get BA Pemberi Hibah BBM Dengan Instansi Lain data via AJAX
     */
    public function getData(Request $request)
    {
        $query = BbmKapaltrans::with(['kapal'])
            ->where('status_ba', 16); // BA Pemberi Hibah BBM Dengan Instansi Lain
    ...
    public function getKapalData(Request $request)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'pangkat_nahkoda' => $kapal->jabatan_nakoda, // Map jabatan_nakoda ke pangkat_nahkoda
    ...
    /**
     * Get BA data (BA Penerimaan BBM) untuk LINK BA dan volume
     */
    public function getBaData(Request $request)
    {
        ...
        if (!$tanggalSurat || !$kapalId) {
            return response()->json([
                'success' => false,
                'message' => 'Tanggal surat dan kapal ID diperlukan'
            ], 400);
        }

        // Cari BA Penerimaan BBM (status_ba = 2) berdasarkan tanggal dan kapal
        $ba = BbmKapaltrans::where('status_ba', 2)
            ->where('tanggal_surat', $tanggalSurat)
            ->whereHas('kapal', function ($query) use ($kapalId) {
                $query->where('m_kapal_id', $kapalId);
            })
        ...
        if (!$ba) {
            return response()->json([
                'success' => false,
                'message' => 'BA Penerimaan BBM tidak ditemukan untuk tanggal dan kapal tersebut'
            ], 404);
        }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'nama_kapal_penerima' => 'required|string|max:100',
            'jabatan_staf_pangkalan' => 'nullable|string|max:30',
            'nama_petugas' => 'nullable|string|max:30',
            'nip_petugas' => 'nullable|string|max:20',
        ...
        try {
            ...
            $ba = BbmKapaltrans::create([
                ...
                'status_ba' => 16, // BA Pemberi Hibah BBM Dengan Instansi Lain
                'kapal_code_temp' => $request->nama_kapal_penerima,
                'jabatan_staf_pangkalan' => $request->jabatan_staf_pangkalan,
                'nama_staf_pagkalan' => $request->nama_petugas,
                'nip_staf' => $request->nip_petugas,
                'link_modul_ba' => $request->link_modul_ba,
                'nomer_persetujuan' => $persetujuan->nomor_persetujuan,
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Pemberi Hibah BBM Dengan Instansi Lain berhasil dibuat',
                'data' => $ba->load('kapal')
            ]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    {
        try {
            $baPemberiHibah = BbmKapaltrans::where('status_ba', 16)
                ->with(['kapal.upt'])
                ->find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Pemberi Hibah BBM Dengan Instansi Lain berhasil diperbarui',
    ...
    public function destroy($id)
    {
        try {
            // Find the record first
            $baPemberiHibah = BbmKapaltrans::where('status_ba', 16)->find($id);
    ...
    public function generatePdf($id)
    {
        try {
            ...
            // Get persetujuan data
            $persetujuan = MPersetujuan::where('deskripsi_persetujuan', $data->link_modul_ba)->first();
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="center">
                            <font size="12"><b><u>BERITA ACARA PEMBERIAN HIBAH BBM DENGAN KAPAL INSTANSI LAIN</b></u></font>
                        </td>
                    </tr>
                    <tr>
                        <td width="100%" align="center">
            ...
            $html .= '<table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    ...
                    <tr>
                        ...
                        Nomor ' . $data->nomer_persetujuan . ' tanggal ' . $this->indo_date($data->tgl_persetujuan) . ', telah dilakukan hibah BBM ' . $data->keterangan_jenis_bbm . ' dari PIHAK II ke PIHAK I sebanyak <b>' . number_format($data->volume_pemakaian) . '</b> liter. Adapun hibah BBM ini di kerenakan <b>' . ($data->sebab_temp ?? '-') . '</b>.
                        </td>
                    </tr>
                </table>';
            ...
            $html .= '<br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    ...
                    <tr>
                        <td width="100%" align="justify">Demikian Berita Acara Pemberian Hibah BBM ini dibuat dengan sebenar – benarnya untuk dapat dijadikan sebagai bahan keterangan dan dipergunakan sebagaimana mestinya.
                        </td>
                    </tr>
                </table>';
            ...
            // Generate filename
            $filename = 'BA_Pemberi_Hibah_BBM_Dengan_Instansi_Lain_' . str_replace('/', '_', $data->nomor_surat) . '_' . date('Y-m-d_H-i-s');
    ...
    public function uploadDocument(Request $request, $id)
    {
        ...
        try {
            $ba = BbmKapaltrans::where('status_ba', 16)->findOrFail($id);
