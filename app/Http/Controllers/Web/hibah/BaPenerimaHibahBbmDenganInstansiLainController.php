...
class BaPenerimaHibahBbmDenganInstansiLainController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('ba-penerima-hibah-bbm-dengan-instansi-lain.index', compact('kapals', 'persetujuans'));
    }

    /**
     * Get BA Penerima Hibah BBM Dengan Instansi Lain data via AJAX
     */
    public function getData(Request $request)
    {
        $query = BbmKapaltrans::with(['kapal'])
            ->where('status_ba', 18); // BA Penerima Hibah BBM Dengan Instansi Lain
    ...
    public function getKapalData(Request $request)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'pangkat_kkm' => $kapal->jabatan_kkm,
            ]
        ]);
    }

    /**
     * Get BA Sebelum Pengisian data for auto-fill
     */
    public function getBaSebelumPengisianData(Request $request)
    {
        ...
        // Get BA Sebelum Pengisian (status_ba = 2) yang belum memiliki link_modul_ba
        // Sesuai dengan logika di project CI
        $baSebelum = BbmKapaltrans::where('status_ba', 2)
            ->where('kapal_code', $kapal->code_kapal)
            ->where('tanggal_surat', $tanggal)
            ->where(function ($query) {
                $query->whereNull('link_modul_ba')
                    ->orWhere('link_modul_ba', '');
            })
        ...
        if (!$baSebelum) {
            return response()->json([
                'success' => false,
                'message' => 'BA Sebelum Pengisian tidak ditemukan untuk tanggal dan kapal tersebut'
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'data' => [
                'link_ba' => $baSebelum->nomor_surat,
                'volume_sebelum' => $baSebelum->volume_sisa,
            ]
        ]);
    }
    ...
    public function getPersetujuanData(Request $request)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'tanggal_persetujuan' => date('Y-m-d'), // Default tanggal hari ini
            ]
        ]);
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'pangkat_nahkoda_temp' => 'nullable|string|max:50',
        ...
        try {
            ...
            // Calculate volume_sisa
            $volume_sisa = $request->volume_sebelum + $request->volume_pengisian;
            ...
            $ba = BbmKapaltrans::create([
                ...
                'status_ba' => 18, // BA Penerima Hibah BBM Dengan Instansi Lain
                'kapal_code_temp' => $request->nama_kapal_pemberi,
            ...
            // Update link_modul_ba di BA Sebelum Pengisian
            // Sesuai dengan logika di project CI
            if ($request->link_ba) {
                BbmKapaltrans::where('status_ba', 2)
                    ->where('kapal_code', $kapal->code_kapal)
                    ->where('tanggal_surat', $request->tanggal_surat)
                    ->where('nomor_surat', $request->link_ba)
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Penerima Hibah BBM Dengan Instansi Lain berhasil dibuat',
                'data' => $ba->load('kapal')
            ]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    {
        try {
            $baPenerimaHibah = BbmKapaltrans::where('status_ba', 18)
                ->with(['kapal.upt'])
                ->find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Penerima Hibah BBM Dengan Instansi Lain berhasil diperbarui',
    ...
    public function destroy($id)
    {
        try {
            // Find the record first
            $baPenerimaHibah = BbmKapaltrans::where('status_ba', 18)->find($id);
    ...
    public function generatePdf($id)
    {
        try {
            ...
            // Content - Format sesuai CI dengan Pihak I dan Pihak II
            $html .= '<br><br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="center">
                            <font size="12"><b><u>BERITA ACARA PENERIMA HIBAH BBM DENGAN KAPAL INSTANSI LAIN</b></u></font>
                        </td>
                    </tr>
                    <tr>
                        <td width="100%" align="center">
                    ...
                    <tr>
                        ...
                        <td width="20%" align="justify">Jabatan</td>
                        <td width="3%" align="justify">:</td>
                        <td width="50%" align="justify">Nakhoda KP ' . $data->kapal_code_temp . '</td>
                    </tr>
                    <tr>
                        <td width="4%" align="justify"></td>
            ...
            $html .= '<table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    ...
                    <tr>
                        <td width="100%" align="justify">Pada hari ini ' . $tanggalFormatted . ' pukul ' . $jamFormatted . ' ' . $data->zona_waktu_surat . ' bertempat di ' . $data->lokasi_surat . ' telah dilakukan hibah BBM ' . $data->keterangan_jenis_bbm . ' dari PIHAK II ke PIHAK I sebanyak <b>' . number_format($data->volume_pengisian) . '</b> liter. Adapun hibah BBM ini di kerenakan <b>' . ($data->sebab_temp ?? '-') . '</b>.
                        </td>
                    </tr>
                </table>';
            ...
            $html .= '<br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    ...
                    <tr>
                        <td width="100%" align="justify">Demikian Berita Acara Penerimaan Hibah BBM ini dibuat dengan sebenar – benarnya untuk dapat dijadikan sebagai bahan keterangan dan dipergunakan sebagaimana mestinya.
                        </td>
                    </tr>
                </table>';
            ...
            $html .= '<br><br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif; font-size:10px" border="0">
                    ...
                    <tr>
                        <td width="40%" align="center">
                            <b>' . $anNakhodaTemp . ' Nakhoda KP. ' . $data->kapal_code_temp . '</b><br><br><br><br><br>
                    ...
                    <tr>
                        <td width="40%" align="center">
                            <b>' . $anKkmTemp . ' KKM KP. ' . $data->kapal_code_temp . '</b><br><br><br><br><br>
            ...
            // Generate filename
            $filename = 'BA_Penerima_Hibah_BBM_Dengan_Instansi_Lain_' . str_replace('/', '_', $data->nomor_surat) . '_' . date('Y-m-d_H-i-s');
    ...
    public function uploadDocument(Request $request, $id)
    {
        ...
        try {
            $ba = BbmKapaltrans::where('status_ba', 18)->findOrFail($id);
