...
class BaPenggunaanBbmController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('ba-penggunaan-bbm.index', compact('kapals'));
    }

    /**
     * Get BA Penggunaan BBM data via AJAX
     */
    public function getData(Request $request)
    {
        $query = BbmKapaltrans::with(['kapal'])
            ->where('status_ba', 3); // BA Penggunaan BBM
    ...
    public function getBaData(Request $request)
    {
        ...
        try {
            ...
            // Cek apakah sudah ada BA Penggunaan BBM untuk tanggal dan kapal ini
            ...
            if ($existingBa) {
                return response()->json([
                    'success' => false,
                    'message' => 'BA Penggunaan BBM untuk kapal dan tanggal ini sudah ada'
                ], 400);
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $ba = BbmKapaltrans::create([
                ...
                'status_ba' => 3, // BA Penggunaan BBM
                'volume_sisa' => $volumeSisa, // Volume penggunaan
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Penggunaan BBM berhasil dibuat',
                'data' => $ba->load('kapal')
            ]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, BbmKapaltrans $baPenggunaanBbm)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Penggunaan BBM berhasil diperbarui',
    ...
    public function destroy(BbmKapaltrans $baPenggunaanBbm)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'BA Penggunaan BBM berhasil dihapus'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function generatePdf(BbmKapaltrans $baPenggunaanBbm)
    {
        try {
                ...
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="center">
                            <font size="12"><b><u>BERITA ACARA PENGGUNAAN BBM</b></u></font>
                        </td>
                    </tr>
                    <tr>
                        <td width="100%" align="center">
                    ...
                    <tr>
                        <td width="100%" align="justify">Menyatakan bahwa telah menggunakan BBM ' . $data->keterangan_jenis_bbm . ' dengan rincian sebagai berikut :
                        </td>
                    </tr>
                </table>';

            // Format dates
            $tanggalSebelumFormatted = \Carbon\Carbon::parse($data->tanggal_sebelum)->locale('id')->isoFormat('D MMMM YYYY');
            $tanggalPengisianFormatted = $data->tanggal_pengisian && $data->tanggal_pengisian != '1970-01-01' && $data->tanggal_pengisian != '0000-00-00'
                ? \Carbon\Carbon::parse($data->tanggal_pengisian)->locale('id')->isoFormat('D MMMM YYYY')
                : '';
            $tanggalSaatIniFormatted = \Carbon\Carbon::parse($data->tanggal_surat)->locale('id')->isoFormat('D MMMM YYYY');

            // Volume table with detailed breakdown
            $html .= '<br><br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="1">
                    <tr>
                        <td width="69%" align="justify">&nbsp; Volume tangki pengukuran sebelumnya, tanggal ' . $tanggalSebelumFormatted . '</td>
                        <td width="3%" align="center">=</td>
                        <td width="17%" align="right">' . number_format($data->volume_sebelum) . '&nbsp;&nbsp;&nbsp;</td>
                        <td width="auto" align="justify">&nbsp;Liter</td>
                    </tr>
                    <tr>
                        <td width="69%" align="justify">&nbsp; Pengisian' . ($tanggalPengisianFormatted ? ', tanggal ' . $tanggalPengisianFormatted : '') . '</td>
                        <td width="3%" align="center">=</td>
                        <td width="17%" align="right">' . number_format($data->volume_pengisian) . '&nbsp;&nbsp;&nbsp;</td>
                        <td width="auto" align="justify">&nbsp;Liter</td>
                    </tr>
                    <tr>
                        <td width="69%" align="justify">&nbsp; Volume tangki pengukuran saat ini, tanggal ' . $tanggalSaatIniFormatted . '</td>
                        <td width="3%" align="center">=</td>
            ...
            $html .= '<br><br>
                <table cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, Helvetica, sans-serif;font-size:12px" border="0">
                    <tr>
                        <td width="100%" align="justify">Demikian Berita Acara Penggunaan BBM ini dibuat dengan sebenar – benarnya untuk dapat dipergunakan sebagaimana mestinya.
                        </td>
                    </tr>
                </table>';
            ...
            // Generate filename
            $filename = 'BA_Penggunaan_BBM_' . str_replace('/', '_', $data->nomor_surat) . '_' . date('Y-m-d_H-i-s');
    ...
    public function uploadDocument(Request $request, $id)
    {
        ...
        try {
            $ba = BbmKapaltrans::where('status_ba', 3)->findOrFail($id);
    ...
    /**
     * Get data BA untuk auto-fill form (sesuai flow CI)
     */
    public function getDataBa(Request $request)
    {
        try {
            $tanggal_surat = date("Y-m-d", strtotime($request->tanggal_surat));
            $m_kapal_id = $request->m_kapal_id;

            // Cek status BA (9) - Akhir Bulan
            $statusBaQuery = BbmKapaltrans::join('m_kapal', 'm_kapal.code_kapal', '=', 'bbm_kapaltrans.kapal_code')
                ->where('tanggal_surat', $tanggal_surat)
                ->where('status_ba', 9)
                ->where('m_kapal.m_kapal_id', $m_kapal_id)
                ->orderBy('jam_surat', 'desc')
                ->first();

            $status_ba = $statusBaQuery ? $statusBaQuery->status_ba : 0;

            // Cari data BA normal (1,2,6,7) pada tanggal yang sama
            $baQuery = BbmKapaltrans::join('m_kapal', 'm_kapal.code_kapal', '=', 'bbm_kapaltrans.kapal_code')
                ->where('tanggal_surat', $tanggal_surat)
                ->whereIn('status_ba', [1, 2, 6, 7])
            ...
            if ($baQuery) {
                ...
                $data['nomor_surat'] = $baQuery->nomor_surat;
                $data['volume_pemakaian'] = number_format($baQuery->volume_sisa);
                $data['volume_sisa'] = number_format($baQuery->volume_sisa);
            } else {
                $data['jml'] = 0;
                $data['nomor_surat'] = '';
            ...
            // Cari data BA sebelumnya
            $baSebelumQuery = BbmKapaltrans::join('m_kapal', 'm_kapal.code_kapal', '=', 'bbm_kapaltrans.kapal_code')
                ->where('tanggal_surat', '<', $tanggal_surat)
                ->whereIn('status_ba', [1, 2, 6, 7])
                ->where('link_modul_ba', '')
                ->where('m_kapal.m_kapal_id', $m_kapal_id)
            ...
            if ($baSebelumQuery) {
                $data['volume_sebelum'] = number_format($baSebelumQuery->volume_sisa);
                $data['tanggal_surat'] = date("d-m-Y", strtotime($baSebelumQuery->tanggal_surat));
            } else {
                $data['volume_sebelum'] = 0;
                $data['tanggal_surat'] = '';
            }

            // Jika status_ba = 9 (Akhir Bulan), cari data BA akhir bulan
            if ($status_ba == 9) {
                $baAkhirBulanQuery = BbmKapaltrans::join('m_kapal', 'm_kapal.code_kapal', '=', 'bbm_kapaltrans.kapal_code')
                    ->where('tanggal_surat', $tanggal_surat)
                    ->where('status_ba', 9)
                    ->where('link_modul_ba_tamb', '')
                    ->where('m_kapal.m_kapal_id', $m_kapal_id)
                    ->orderBy('jam_surat', 'desc')
                    ->first();

                if ($baAkhirBulanQuery) {
                    $volume_sisa = floatval($baAkhirBulanQuery->volume_sebelum) - floatval($baAkhirBulanQuery->volume_pemakaian) - floatval($baAkhirBulanQuery->penggunaan);

                    $data['jml'] = 1;
                    $data['nomor_surat'] = $baAkhirBulanQuery->nomor_surat;
                    $data['tanggal_surat'] = date("d-m-Y", strtotime($baAkhirBulanQuery->tanggal_sebelum));
                    $data['volume_pemakaian'] = number_format($baAkhirBulanQuery->volume_pemakaian);
                    $data['volume_sebelum'] = number_format($baAkhirBulanQuery->volume_sebelum);
                    $data['volume_sisa'] = number_format($volume_sisa);
                } else {
                    $data['jml'] = 0;
                    $data['nomor_surat'] = '';
                    $data['tanggal_surat'] = '';
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data BA: ' . $e->getMessage()
            ], 500);
        }
    }
}
