...
class BaPenerimaanBbmController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        // Update field foto dengan URL lengkap untuk setiap item
        $items = $data->items();
        foreach ($items as $item) {
            if ($item->foto_so) {
                $item->foto_so = asset($item->foto_so);
            }
            foreach ($item->transdetails as $detail) {
                if ($detail->foto_do) {
                    $detail->foto_do = asset($detail->foto_do);
                }
                if ($detail->foto_segel) {
                    $detail->foto_segel = asset($detail->foto_segel);
                }
                if ($detail->foto_volume) {
                    $detail->foto_volume = asset($detail->foto_volume);
                }
            }
        }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            // Foto validasi
            'foto_so' => 'nullable|file|image|max:10240', // Max 10MB
            'foto_do' => 'nullable|array',
            'foto_do.*' => 'nullable|file|image|max:10240',
            'foto_segel' => 'nullable|array',
            'foto_segel.*' => 'nullable|file|image|max:10240',
            'foto_volume' => 'nullable|array',
            'foto_volume.*' => 'nullable|file|image|max:10240',
        ]);
        ...
        try {
            ...
            // Upload foto SO jika ada
            $fotoSoPath = null;
            if ($request->hasFile('foto_so')) {
                $fotoSoPath = $this->uploadFoto($request->file('foto_so'), 'so');
            }
            ...
            for ($i = 0; $i < count($request->transportasi); $i++) {
                if (!empty($request->transportasi[$i])) {
                    ...
                    // Upload foto untuk setiap transportasi
                    $fotoDoPath = null;
                    $fotoSegelPath = null;
                    $fotoVolumePath = null;

                    if (isset($request->foto_do[$i]) && $request->hasFile('foto_do.' . $i)) {
                        $fotoDoPath = $this->uploadFoto($request->file('foto_do.' . $i), 'do');
                    }
                    if (isset($request->foto_segel[$i]) && $request->hasFile('foto_segel.' . $i)) {
                        $fotoSegelPath = $this->uploadFoto($request->file('foto_segel.' . $i), 'segel');
                    }
                    if (isset($request->foto_volume[$i]) && $request->hasFile('foto_volume.' . $i)) {
                        $fotoVolumePath = $this->uploadFoto($request->file('foto_volume.' . $i), 'volume');
                    }
                    ...
                    BbmTransdetail::create([
                        ...
                        'foto_do' => $fotoDoPath,
                        'foto_segel' => $fotoSegelPath,
                        'foto_volume' => $fotoVolumePath,
                        'tanggalinput' => now(),
            ...
            $ba->load(['kapal', 'transdetails']);

            // Add full URLs to all foto fields
            $this->addFileUrls($ba);
    ...
    public function show($id)
    {
        try {
            ...
            // Add full URLs to all foto fields
            $this->addFileUrls($baPenerimaanBbm);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Upload foto SO jika ada
            $fotoSoPath = $baPenerimaanBbm->foto_so; // Keep existing foto if no new one
            if ($request->hasFile('foto_so')) {
                // Delete old foto if exists
                if ($baPenerimaanBbm->foto_so && file_exists(public_path($baPenerimaanBbm->foto_so))) {
                    unlink(public_path($baPenerimaanBbm->foto_so));
                }
                $fotoSoPath = $this->uploadFoto($request->file('foto_so'), 'so');
            }

            // Handle file uploads untuk foto dan ttd
            $fotoPath = $baPenerimaanBbm->foto; // Keep existing foto if no new one
            $ttdPath = $baPenerimaanBbm->ttd; // Keep existing ttd if no new one

            if ($request->hasFile('foto')) {
                // Delete old foto if exists
                if ($baPenerimaanBbm->foto && file_exists(public_path('uploads/ba-penerimaan-bbm/' . $baPenerimaanBbm->foto))) {
                    unlink(public_path('uploads/ba-penerimaan-bbm/' . $baPenerimaanBbm->foto));
                }
                $fotoPath = $this->uploadFile($request->file('foto'), 'foto', $baPenerimaanBbm->trans_id);
            }

            if ($request->hasFile('ttd')) {
                // Delete old ttd if exists
                if ($baPenerimaanBbm->ttd && file_exists(public_path('uploads/ba-penerimaan-bbm/' . $baPenerimaanBbm->ttd))) {
                    unlink(public_path('uploads/ba-penerimaan-bbm/' . $baPenerimaanBbm->ttd));
                }
                $ttdPath = $this->uploadFile($request->file('ttd'), 'ttd', $baPenerimaanBbm->trans_id);
            }
            ...
            // Load data dengan relasi
            $baPenerimaanBbm->load(['kapal', 'transdetails']);
     ...
     */
    private function uploadFoto($file, $prefix = 'foto')
    {
        if (!$file) {
            return null;
        }
        ...
        $storedName = $prefix . '_' . Str::slug($fileName) . '_' . time() . '_' . Str::random(8) . '.' . $extension;

        // Path penyimpanan di public
        $path = public_path('uploads/ba-foto/' . date('Y/m/d'));
        ...
        return $storedName; // Return hanya nama file, bukan URL lengkap
    }

    /**
     * Upload file helper method for foto and ttd
     */
    private function uploadFile($file, $type, $transId)
    {
        $extension = $file->getClientOriginalExtension();
        $filename = $transId . '_' . time() . '.' . $extension;
        $file->move(public_path('uploads/ba-penerimaan-bbm'), $filename);
        return $filename;
    }
    ...
    {
        // Handle foto and ttd fields
        if ($data->foto) {
            $data->foto = url('uploads/ba-penerimaan-bbm/' . $data->foto);
        }
        if ($data->ttd) {
            $data->ttd = url('uploads/ba-penerimaan-bbm/' . $data->ttd);
        }

        // Handle foto_so field
        if ($data->foto_so) {
            $data->foto_so = url('uploads/ba-foto/' . date('Y/m/d', strtotime($data->created_at)) . '/' . $data->foto_so);
        }

        // Handle transdetails foto fields
        if ($data->transdetails) {
            foreach ($data->transdetails as $detail) {
                if ($detail->foto_do) {
                    $detail->foto_do = url('uploads/ba-foto/' . date('Y/m/d', strtotime($detail->tanggalinput)) . '/' . $detail->foto_do);
                }
                if ($detail->foto_segel) {
                    $detail->foto_segel = url('uploads/ba-foto/' . date('Y/m/d', strtotime($detail->tanggalinput)) . '/' . $detail->foto_segel);
                }
                if ($detail->foto_volume) {
                    $detail->foto_volume = url('uploads/ba-foto/' . date('Y/m/d', strtotime($detail->tanggalinput)) . '/' . $detail->foto_volume);
                }
            }
        }
    }
