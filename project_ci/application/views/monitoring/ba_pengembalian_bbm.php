		...
		<form id="dataSeri">
			...
			<div class="row">
				  <div class="col-md-3 ">
					...
					<input type="text" name="tanggal_surat_penitip" id="tanggal_surat_penitip" class="form-control datepicker" onChange="getValPenitip()">
				 </div>
				 
				 	<div class="col-md-5 ">
					<div class="form-group">
						<label>NOMOR BA PENITIPAN</label>
						<input type="text" name="nomor_surat_penitip" id="nomor_surat_penitip" class="form-control" disabled value="">
					</div>
				</div>	
			</div>
            ...
            <div class="row">
			    <div class="col-md-4">
					...
					<input type="text" placeholder="" id="penyedia_penitip" name="penyedia_penitip" class="form-control" value="">
				 </div>
				 <div class="col-md-6">
					<label>ALAMAT PENYEDIA</label>
					<input type="text" placeholder="" id="alamat_penyedia_penitip" name="alamat_penyedia_penitip" class="form-control" value="">
				 </div>
            </div></br>
            <div class="row">                 
			    <div class="col-md-4">
					<label>NAMA </label>
					<input type="text" placeholder="" id="nama_penitip" name="nama_penitip" class="form-control" value="">
				 </div>
                 <div class="col-md-4">
					<label>JABATAN</label>
					<input type="text" placeholder="" id="jabatan_penitip" name="jabatan_penitip" class="form-control" value="">
				 </div>           
			    
				<div class="col-md-8 ">
					<label>ALAMAT PENITIPAN</label>
					<input type="text" placeholder="" id="alamat_penitip" name="alamat_penitip" class="form-control" value="">
				 </div>
			...
			<div class="row">
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="volume_sebelum" id="volume_sebelum" class="form-control angka" value="" onkeypress="validate(event)" disabled>
					</div>
				</div>
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="penggunaan" id="penggunaan" class="form-control angka" value="" onkeypress="validate(event)" disabled>
					</div>
				</div>
             ...
             <div class="row">
                ...
                <div class="col-md-4 ">
					<div class="form-group">
						<label>VOLUME TANGKI BBM PENGUKURAN SAAT INI</label>
						   <input type="text" placeholder="" id="volume_pemakaian" name="volume_pemakaian" class="form-control angka" value="0" onkeypress="validate(event)">
					</div>
				</div>
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="keterangan_jenis_bbm" id="keterangan_jenis_bbm" class="form-control" value="">
					</div>
				</div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_pengembalian_bbm/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_bbm/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_bbm/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_pengembalian_bbm/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function createDok(){
    ...
    $("#nomor_surat_penitip").removeAttr("disabled");
	$("#penggunaan").removeAttr("disabled");
	$("#volume_sebelum").removeAttr("disabled");
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_pengembalian_bbm/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_pengembalian_bbm/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
...
function getValPenitip(){
    
    var tanggal_surat_penitip = $("#tanggal_surat_penitip").val();
    
    var m_kapal_id = $("#m_kapal_id").val();
    
    var dataForm = {
        'tanggal_surat_penitip' : tanggal_surat_penitip,
        'm_kapal_id' : m_kapal_id
    }
    
    $.ajax({
		type : "POST",
		url	: "<?php echo base_url()?>index.php/monitoring/ba_pengembalian_bbm/getDataPenitip",
		data: dataForm,
		dataType: "json",
		success: function (data) {
		    
		    if(tanggal_surat_penitip != ''){
		        
		        if(data.jml == 0){
			        alert('Tidak ada penitipan ');
			        
    			    $("#nomor_surat_penitip").val(data.nomor_surat);
