...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_hibah_bbm_instansi_lain/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_hibah_bbm_instansi_lain/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_hibah_bbm_instansi_lain/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_hibah_bbm_instansi_lain/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function validateVolume(x){
	...
	if(x==1){
		...
		var volume_sebelum   = parseInt($('#volume_sebelumEd').val().replace('.', ''));
		var volume_pemakaian = parseInt($('#volume_pemakaianEd').val().replace('.', ''));
		...
		var volume_sebelum   = parseInt($('#volume_sebelum').val().replace('.', ''));
		var volume_pemakaian = parseInt($('#volume_pemakaian').val().replace('.', ''));
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_hibah_bbm_instansi_lain/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	if(volume_pemakaianEd == 0){
		...
		$.ajax({
			type : 'POST',
			url: '<?php echo WS_JQGRID."monitoring.ba_hibah_bbm_instansi_lain/edit"; ?>',
			data : x,
			dataType : 'json',
			success : function(data){
