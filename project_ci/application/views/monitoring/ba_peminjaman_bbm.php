  ...
  <div class="container" id="tab1C">
	...
	<div class="container" id="tab2C">
		<form id="dataSeri">
			...
			<div class="row">
				...
				<div class="col-md-5">
					<div class="form-group">
						...
						<select class="form-control custom-control" id="m_kapal_id_peminjam" name="m_kapal_id_peminjam" onChange="getValKapalPeminjam(this, 0)">
							<option>--Pilih--</option>
							<?php
								$ci = & get_instance();
			...
			<div class="row">
				 ...
				 <div class="col-md-4 ">
					<label>NAMA NAKHODA Peminjam &nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_nakhoda_temp" name="an_nakhoda_temp" value="1" > An.  </label>
			...
			<div class="row">	
				 ...
				 <div class="col-md-4 ">
					<label>NAMA KKM Peminjam&nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_kkm_temp" name="an_kkm_temp" value="1" > An. </label>
	...
	<div class="container" id="tab3C">
		<form id="dataUpld"  enctype="multipart/form-data">
			...
			<br>
			<button type="button" id="createdokEd_btn" class="btn btn-success mb-xl" onClick="simpanUp()">SIMPAN</button>
		</form>
   </div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_peminjaman_bbm/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_peminjaman_bbm/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_peminjaman_bbm/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_peminjaman_bbm/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function getValKapalPeminjam(val, x){
...
function validateVolume(x){
	...
	if(x==1){
		...
		// ReplaceNumberWithCommas(parseFloat($('#volume_sebelumEd').val().replace(/,/g, '')));
		var volume_pemakaian = parseFloat(document.getElementById('volume_pemakaianEd').value.replace(/,/g ,''));  
		// parseFloat($('#volume_pemakaianEd').val().replace('.', ''));
		
		// alert(volume_pemakaian);
		
		if(volume_pemakaian > volume_sebelum){
			
			alert("Volume Peminjaman Tidak Boleh Melebihi Volume Sounding / Lakukan Sounding Ulang");
		...
		// console.log(volume_sebelum);
		// console.log(volume_pemakaian);
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_peminjaman_bbm/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	if(volume_pemakaianEd == 0){
		...
		$.ajax({
			type : 'POST',
			url: '<?php echo WS_JQGRID."monitoring.ba_peminjaman_bbm/edit"; ?>',
			data : x,
			dataType : 'json',
			success : function(data){
