		...
		<form id="dataSeri">
			...
			<div class="row">
				...
				<div class="col-md-3">
					<div class="form-group">
						...
						<select class="form-control custom-control" id="trans_idHibah" name="trans_idHibah" onChange="getValHibah(this)">
							<option value="0">--Pilih--</option>
							<?php
								$ci = & get_instance();
								
								$sql = "SELECT trans_id, nomor_surat FROM bbm_kapaltrans WHERE kapal_code_temp = (
										SELECT code_kapal FROM m_kapal WHERE m_kapal_id IN (SELECT m_kapal_id FROM sys_user_kapal WHERE conf_user_id = '".$this->session->userdata('userid')."' )) AND status_ba = 14 AND status_temp = 0";
								// var_dump($sql);
				...
				<div class="col-md-3 ">
					...
					<input type="text" name="tanggal_hibah" id="tanggal_hibah" class="form-control datepicker" readonly="">
				 </div>
			...
			<div class="row">
			    ...
			    <div class="col-md-5">
					...
					<input type="text" placeholder="" id="keterangan_jenis_bbm" name="keterangan_jenis_bbm" class="form-control" value="" readonly="">
				 </div>
			...
			<br>
				 <div class="col-md-4 ">
					<label>NAMA NAKHODA Pemberi HIBAH &nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_nakhoda_temp" name="an_nakhoda_temp" value="1"> An.  </label>
			...
			<br>
				 <div class="col-md-4 ">
					<label>NAMA KKM Pemberi HIBAH &nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_kkm_temp" name="an_kkm_temp" value="1"> An. </label>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_kapal_pengawas/getDataTable',
...
function getTab3(trans_id){
	...
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_kapal_pengawas/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_kapal_pengawas/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_penerima_hibah_kapal_pengawas/getDataLihat/'+trans_id);
	$('#tab5').click();
}
	...
	if(tanggal_surat == ''){
		...
		$.ajax({
			...
			success: function (data) {	
				...
				$("#tanggal_hibah").val(data.tanggal_surat);
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_penerima_hibah_kapal_pengawas/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_penerima_hibah_kapal_pengawas/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
