	...
	<div class="container" id="tab2C">
		<form id="dataSeri">
			...
			<div class="row">
				...
				<div class="col-md-7 ">
					<div class="form-group">
						<label>SISA BBM AKHIR BULAN SEBELUMNYA</label>
				...
				<div class="col-md-7 ">
					<div class="form-group">
						<label>TANGGAL BA AKHIR BULAN SEBELUMNYA</label>
				...
				<div class="col-md-7 ">
					<div class="form-group">
						<label>VOLUME BBM AKHIR BULAN SAAT INI</label>
						   <input type="text" placeholder="" id="volume_sisa" name="volume_sisa" class="form-control angka" value="0" onkeypress="validate(event)" onkeyup="getPenggunaan()">
					</div>
				</div>
				...
				<div class="col-md-7 ">
					<div class="form-group">
						<label>VOLUME PEMAKAIAN</label>
						   <input type="text" placeholder="" id="volume_pemakaian" name="volume_pemakaian" class="form-control angka" value="0" disabled>
					</div>
				</div>
			</div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_akhir_bulan/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_akhir_bulan/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_akhir_bulan/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_akhir_bulan/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function createDok(){
	...
	if(nomor_surat == ''){
	    ...
	    $.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_akhir_bulan/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_akhir_bulan/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
...
function getPenggunaan(){
	...
	var res = 0;
	document.getElementById('volume_pemakaian').value = res;
	
	document.getElementById('volume_pemakaian').value = ReplaceNumberWithCommas(parseFloat(document.getElementById('volume_sebelum').value.replace(/,/g ,''))+parseFloat(document.getElementById('volume_pengisian').value.replace(/,/g ,'')) - parseFloat(document.getElementById('volume_sisa').value.replace(/,/g ,'')));
	
}

function getPenggunaanx(){
	
	document.getElementById('volume_pemakaianx').value = ReplaceNumberWithCommas(parseFloat(document.getElementById('volume_sebelumx').value.replace(/,/g ,''))+parseFloat(document.getElementById('volume_pengisianx').value.replace(/,/g ,'')) - parseFloat(document.getElementById('volume_sisax').value.replace(/,/g ,'')));
