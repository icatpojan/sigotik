		...
		<form id="dataSeri">
			...
			<div class="row">
				<div class="col-md-3 ">
					<div class="form-group">
						...
						<select class="form-control custom-control" id="code_upt" name="code_upt" onChange="getUpt()">
							<option>--Pilih--</option>
							<?php
								$ci = & get_instance();
								
								$sql = "SELECT * FROM m_upt a
								...
								$query = $ci->db->query($sql);
								foreach ($query->result() as $list){
									echo '<option value="'.$list->code.'">'.$list->nama.'</option>';
								}
							?>
				...
				<div class="col-md-3 ">
					<div class="form-group">
						...
						<input type="text" id="instansi_temp" name="instansi_temp" class="form-control" />
					</div>
				</div>
				...
				<div class="col-md-3 ">
					<div class="form-group">
						...
						<textarea id="alamat_instansi_temp" name="alamat_instansi_temp" rows="3" cols="50" class="form-control"/>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="penyedia" name="penyedia" class="form-control">
					</div>
				</div>
				...
				<div class="col-md-4 ">
					<div class="form-group">
						...
						<input type="text" name="keterangan_jenis_bbm" name="keterangan_jenis_bbm" class="form-control" value="BIO SOLAR">
					</div>
				</div>
			 </div>  
			...
			<div class="row">
				<div class="col-md-7 ">
					<div class="form-group">
						<label>NO SO</label>
						<input type="text" name="no_so" id="no_so" class="form-control" value="">
					</div>
				</div>	
			</div>
				 ...
				 <div class="col-md-11">
					...
					<table id="myTable" class="table">
					 <tr>
						...
						<th>VOLUME</th>
						<th>KETERANGAN</th>
						<th><button type="button"  class="btn btn-info" onclick="myFunction()">ADD</button></th>
						<th><button type="button"  class="btn btn-danger" onclick="myDel()">DEL</button></th>
					  </thead>
					</tr>
					  <tr>
						<td><input type="text" name="transportasi[]" id="transportasi" class="form-control" value=""></td>
						<td><input type="text" name="no_do[]" id="no_do" class="form-control" value=""></td>
						<td><input type="text" name="volume_isi[]" id="volume_isi" class="form-control angka" value="0" onKeyUp="numericFilter(this);"></td>
						<td><input type="text" name="keterangan[]" id="keterangan" class="form-control" value=""></td>
					  </tr>
					   <tr>
						<td><input type="text" name="transportasi[]" id="transportasi" class="form-control" value=""></td>
						<td><input type="text" name="no_do[]" id="alamat1" class="form-control" value=""></td>
						<td><input type="text" name="volume_isi[]" id="volume_isi" class="form-control angka" value="0" onKeyUp="numericFilter(this);"></td>
						<td><input type="text" name="keterangan[]" id="keterangan" class="form-control" value=""></td>
					  </tr>
					   <tr>
			...
			<br>
				...
				<div class="col-md-4 ">
					...
					<input type="text" placeholder="" id="nama_penyedia" name="nama_penyedia" class="form-control" value="">
				 </div>
...
$(document).ready(function() {
	...
	$('#data_tables').dataTable({
		...
		'sAjaxSource'    : '<?php echo base_url()?>index.php/monitoring/ba_penerimaan_hibah_bbm/getDataTable',
...
function getTab3(trans_id){
	// alert(nomor_surat);
	$('#tab3').show();
	$('#dataUlang').load('<?php echo base_url()?>index.php/monitoring/ba_penerimaan_hibah_bbm/getDataUpload/'+trans_id);
	$('#tab3').click();
}

function getTab4(trans_id){
	$('#tab4').show();
	$('#dataRubah').load('<?php echo base_url()?>index.php/monitoring/ba_penerimaan_hibah_bbm/getDataEdit/'+trans_id);
	$('#tab4').click();
}

function getTab5(trans_id){
	$('#tab5').show();
	$('#dataLihat').load('<?php echo base_url()?>index.php/monitoring/ba_penerimaan_hibah_bbm/getDataLihat/'+trans_id);
	$('#tab5').click();
}
...
function getVolumeHibah(){
    ...
    $.ajax({
		type : "POST",
		url	: "<?php echo base_url()?>index.php/monitoring/ba_pemberi_hibah_bbm_kapal_pengawas/getDataBa",
		data: dataForm,
		dataType: "json",
		success: function (data) {
		    ...
		    if(tanggal_surat != ''){
		        ...
		        if(data.jml == 0){
    			    ...
    			    $("#volume_sisa").val(data.volume_pemakaian);
    			    
    			    $("#createdok").attr("disabled", true);
    ...
    $.ajax({
		...
		success: function (data) {
		    ...
		    if(tanggal_surat != ''){
		        ...
		        if(data.jml == 0){
			        alert('Hari ini anda belum melakukan sounding/pengukuran');
...
function myFunction() {
  ...
  cell1.innerHTML = '<input type="text" name="transportasi[]" id="transportasi" class="form-control" value="">';
  cell2.innerHTML = '<input type="text" name="no_do[]" id="alamat1" class="form-control" value="">';
  cell3.innerHTML = '<input type="text" name="volume_isi[]" id="volume_isi" class="form-control angka" value="0" onKeyUp="numericFilter(this);">';
  cell4.innerHTML = '<input type="text" name="keterangan[]" id="keterangan" class="form-control" value="">';
}
...
function myFunctionx() {
  var table = document.getElementById("myTablex");
  var rowCount = $('#myTablex tr').length;
  // alert(rowCount);
  var row = table.insertRow(rowCount);
  var cell1 = row.insertCell(0);
  var cell2 = row.insertCell(1);
  var cell3 = row.insertCell(2);
  var cell4 = row.insertCell(3);
  cell1.innerHTML = '<input type="text" name="transportasix[]" id="transportasi" class="form-control" value="">';
  cell2.innerHTML = '<input type="text" name="no_dox[]" id="no_dox" class="form-control" value="">';
  cell3.innerHTML = '<input type="text" name="volume_isix[]" id="volume_isi" class="form-control angka" value="0" onKeyUp="numericFilter(this);">';
  cell4.innerHTML = '<input type="text" name="keteranganx[]" id="keterangan" class="form-control" value="">';
}
...
function myDelx(){
	...
	// alert(rowCount);
	if(rowCount != 1){
		document.getElementById("myTablex").deleteRow(rowCount-1);
	}
...
function getUpt(){
	...
	if(code_upt == '999'){
		...
		$.ajax({
			type : "POST",
			url	: "<?php echo base_url()?>index.php/monitoring/ba_penerimaan_hibah_bbm/getUpt",
			data: {code_upt:code_upt},
			dataType: "json",
			success: function (data) {	
			
				$("#alamat_instansi_temp").val(data.alamat1);
				$("#instansi_temp").val(data.nama);
...
function getUptEd(){
	...
	$.ajax({
		...
		success: function (data) {	
			...
			$("#alamat_instansi_tempEd").val(data.alamat1);
			$("#instansi_tempEd").val(data.nama);
...
function createDok(){
	...
	// var p = validateVolume();
	
	var volume_sisa = $('#volume_sisa').val();
	...
	if(nomor_surat == ''){
		...
		$.ajax({
    		type : 'POST',
    		url: '<?php echo WS_JQGRID."monitoring.ba_penerimaan_hibah_bbm/create"; ?>',
    		data : x,
    		dataType : 'json',
    		success : function(data){
...
function editDok(){
	...
	$.ajax({
		type : 'POST',
		url: '<?php echo WS_JQGRID."monitoring.ba_penerimaan_hibah_bbm/edit"; ?>',
		data : x,
		dataType : 'json',
		success : function(data){
