...
class Lap_cetak extends CI_Controller {
            ...
            $sanggarans = 0;
				...
				$x.='
							<tr>
                                ...
                                <td style="text-align:right;">Rp. '.number_format($anggarans2,0,',','.').'</td>
							</tr>
				';
				$i++;
				//$total+=$list->anggaran;
				...
				$x.='
							<tr>
								...
								<td style="text-align:right;">Rp. '.number_format($sanggarans,0,',','.').'</td>
							</tr>
				';
			}else{
				$x.='
			...
			if($jml > 0){
				...
				$x.='
							<tr>
								...
								<td style="text-align:right;">Rp. '.number_format($anggaran,0,',','.').'</td>
								<td style="text-align:right;">Rp. '.number_format($perubahan,0,',','.').'</td>								
							</tr>
				';
				$i++;
	...
	function xxx_showdtotalpakes($tgl_awal, $tgl_akhir){
		$ci = & get_instance();
		$code = $ci->session->userdata('m_upt_code');
		...
		if($jum > 0){
			...
			foreach($sql->result() AS $list){
				...
				$sisa_anggaran = $anggaran - $list->tagihan;
				if($list->tagihan<=0 or $anggaran<=0){
					$persentase = 0;
				} else {
					$persentase = ($list->tagihan / $anggaran) * 100;
				}
			$x.='
						<tr>
							<td>'.$i.'</td>
		...
		if($jum > 0){
			...
			$x.='
						<tr>
							...
							<td>'.$qtabe['nama'].'</td>
							<td style="text-align:right;">Rp. '.number_format($anggarans2,0,',','.').'</td>
							<td style="text-align:right;">Rp. '.number_format($tagihan,0,',','.').'</td>
							<td style="text-align:right;">Rp. '.number_format($sisa_anggaran,0,',','.').'</td>
							<td style="text-align:right;">'.number_format($persentase,2,',','.').' %</td>
						</tr>
			';
	...
	function showdtotalpakaibbm($tgl_awal,$tgl_akhir,$codes,$codekpl){
		...
		<table cellpadding=0 cellspacing=0 width="100%" >
				<tr>
					...
					<td><b>
						<p class = "small" align=center style="text-align:center"> <span style="font-size:14.0pt;color:#0000E1">LAPORAN TOTAL PENERIMAAN DAN PENGGUNAAN BBM</span></p>
						<p class = "small" align=center style="text-align:center"> <span lang=SV style="color:#0000E1">DIREKTORAT PEMANTAUAN DAN ARMADA</span></p>';
                		if($judul_namaupt != '') {
	...
	function prevpinjamanbbm_print($tgl_awal,$tgl_akhir,$codes,$codekpl){
		$ci = & get_instance();
		$tgl_awal = date('Y-m-d', strtotime($tgl_awal));
		$tgl_akhir = date('Y-m-d', strtotime($tgl_akhir));
		$tahun = date('Y',strtotime($tgl_awal));
	...
	function prevhibahantarkapal_print($tgl_awal,$tgl_akhir,$codes,$codekpl){
		$ci = & get_instance();
		$tgl_awal = date('Y-m-d', strtotime($tgl_awal));
		$tgl_akhir = date('Y-m-d', strtotime($tgl_akhir));
		$tahun = date('Y',strtotime($tgl_awal));
		...
		if($cek){
			...
			$sql = $ci->db->query("
						...
						SELECT kapal_code AS nama_kapal,tanggal_surat,a.nomor_surat, volume_sebelum, volume_pemakaian, kapal_code_temp, status_temp, sebab_temp from
							(SELECT kapal_code,tanggal_surat,a.nomor_surat, volume_sebelum, kapal_code_temp, status_temp, sebab_temp FROM bbm_kapaltrans a WHERE status_ba = '14' AND tanggal_surat >= '".$tgl_awal."' AND tanggal_surat <= '".$tgl_akhir."' GROUP BY nomor_surat) a,
							(SELECT code_kapal, nama_kapal, m_kapal_id ,m_upt_code FROM m_kapal ) b
							WHERE a.kapal_code_temp = b.code_kapal  ".$kond."  GROUP BY nomor_surat
				");
