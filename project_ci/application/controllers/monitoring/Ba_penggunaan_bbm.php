...
class Ba_penggunaan_bbm extends CI_Controller {
	...
	public function getDataTable(){
		...
		$sql = "SELECT * 
					FROM bbm_kapaltrans a WHERE status_ba = 3
	...
	function getDataBa(){
    	...
    	$sqlGet = "SELECT * FROM `bbm_kapaltrans` 
		            JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
		    WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (9) AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." 
		    ORDER BY jam_surat DESC LIMIT 1";
    	$qGet = $ci->db->query($sqlGet);
		$tGet = $qGet->num_rows();
    	
    	if($tGet>0){
        	foreach ($qGet->result() as $listGet){
            	$status_ba = $listGet->status_ba;
            }
        }else{
			$status_ba = 0;
		}
   		
    	// var_dump($status_ba);die();
    
		$sql = "SELECT * FROM `bbm_kapaltrans` 
		            JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
		    WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (1,2,6,7) AND link_modul_ba = '' AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." 
		    ORDER BY jam_surat DESC LIMIT 1"; 
		...
		if($total>0){
			...
			$data['jml'] = 1;
            // $status_ba = $list->status_ba;
            $data['status_ba'] = $status_ba;
		    $data['nomor_surat'] = $list->nomor_surat;
		    $data['volume_pemakaian'] = number_format($list->volume_sisa);
		    ...
		    $data['jml'] = 0;
		    $data['nomor_surat'] = '';
		    $data['volume_pemakaian'] = 0;
        	$data['volume_sisa'] = 0;
        	
		    
		}
		
		$sql2 = "SELECT * FROM `bbm_kapaltrans` JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code 
		WHERE tanggal_surat < '".$tanggal_surat."' AND status_ba IN (1,2,6,7) AND link_modul_ba = '' AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']."  ORDER BY tanggal_surat, jam_surat DESC LIMIT 1"; 
		...
		if($total2>0){
		    ...
		    $data['volume_sebelum'] = number_format($list2->volume_sisa);
		    $data['tanggal_surat'] =  date("d-m-Y", strtotime($list2->tanggal_surat));
		...
		if($status_ba == 9){
        	...
        	$sql9 = "SELECT * FROM `bbm_kapaltrans` 
		            JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
		    WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (9) AND link_modul_ba_tamb = '' AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." 
		    ORDER BY jam_surat DESC LIMIT 1"; 
        	...
        	if($total9>0){
           		...
           		foreach ($query9->result() as $list9){
                	...
                	$volume_sisa = floatval($list9->volume_sebelum)- floatval($list9->volume_pemakaian) - floatval($list9->penggunaan);
                    // var_dump($volume_sisa);die();
            		$data9['jml'] = 1;
           			$data9['nomor_surat'] = $list9->nomor_surat;
            		$data9['tanggal_surat'] = date("d-m-Y", strtotime($list9->tanggal_sebelum));
            		$data9['volume_pemakaian'] = number_format($list9->volume_pemakaian);
            		$data9['volume_sebelum'] = number_format($list9->volume_sebelum);
                	$data9['volume_sisa'] = number_format($volume_sisa);
            	...
            	$data9['nomor_surat']     = '';
        		$data9['tanggal_surat']   = '';
            	$data9['volume_pemakaian'] = 0;
        		$data9['volume_sebelum']   = 0;
            	$data9['volume_sisa']   = 0;
	...
	function getDataEdit($trans_id){
		...
		if($list->tanggal_pengisian == '0000-00-00'){
		    $tanggal_pengisian = '';
		}else{
		    $tanggal_pengisian = date("d-m-Y", strtotime($list->tanggal_pengisian));
		}
		...
		$ed .= '
			<div class="row">
				...
				<div class="col-md-5 ">
					<div class="form-group">
						<label>LINK BA</label>
						<input type="text" name="link_ba" id="link_ba" class="form-control" value="'.$list->link_modul_ba.'" disabled>
					</div>
				</div>
			...
			<div class="row">
				...
				<div class="col-md-6 ">
					<div class="form-group">
						<label>VOLUME TANGKI PENGUKURAN SEBELUMNYA </label>
						 <input type="text" placeholder="" id="volume_sebelumx" name="volume_sebelumx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_sebelum).'">
					</div>
				</div>
				
				<div class="col-md-4 ">
					<div class="form-group">
						<label>TANGGAL PENGUKURAN SEBELUMNYA </label>
						<input type="text" placeholder="" id="tanggal_sebelum" name="tanggal_sebelum" class="form-control datepicker" value="'.date("d-m-Y", strtotime($list->tanggal_sebelum)).'">
					</div>
				</div>
				
				<div class="col-md-6 ">
					<div class="form-group">
						<label>VOLUME PENGISIAN SEBELUMNYA</label>
						  <input type="text" placeholder="" id="volume_pengisianx" name="volume_pengisianx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_pengisian).'">
					</div>
				</div>
				
				<div class="col-md-4 ">
					<div class="form-group">
						<label>TANGGAL PENGISIAN SEBELUMNYA</label>
						   <input type="text" placeholder="" id="tanggal_pengisian" name="tanggal_pengisian" class="form-control datepicker" value="'.$tanggal_pengisian.'">
					</div>
				</div>
				
				<div class="col-md-5 ">
					<div class="form-group">
						<label>VOLUME TANGKI BBM PENGUKURAN SAAT INI</label>
						   <input type="text" placeholder="" id="volume_pemakaianx" name="volume_pemakaianx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_sisa).'">
					</div>
				</div>
				...
				<div class="col-md-5 ">
					<div class="form-group">
						<label>JUMLAH PENGGUNAAN</label>
						   <input type="text" placeholder="" id="volume_sisax" name="volume_sisax" class="form-control angka" value="'.number_format($list->volume_pemakaian).'" disabled>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-5 ">
					<div class="form-group">
						...
						<input type="text" name="peruntukan" name="peruntukan" class="form-control" value="'.$list->peruntukan.'">
					</div>
				</div>
	...
	function getDataLihat($trans_id){
		...
		$ed .= '
			...
			<div class="row">
				...
				<div class="col-md-6 ">
					<div class="form-group">
						<label>VOLUME TANGKI SEBELUM PENGUKURAN</label>
						 <input type="text" placeholder="" id="volume_sebelumx" name="volume_sebelumx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_sebelum).'" readonly="">
					</div>
				</div>
				
				<div class="col-md-4 ">
					<div class="form-group">
						<label>TANGGAL SEBELUM PENGUKURAN</label>
						<input type="text" placeholder="" id="tanggal_sebelum" name="tanggal_sebelum" class="form-control datepicker" value="'.date("d-m-Y", strtotime($list->tanggal_sebelum)).'" disabled>
					</div>
				</div>
				
				<div class="col-md-6 ">
					<div class="form-group">
						<label>VOLUME TANGKI PENGISIAN SEBELUMNYA</label>
						  <input type="text" placeholder="" id="volume_pengisianx" name="volume_pengisianx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_pengisian).'" readonly="">
					</div>
				</div>
				...
				<div class="col-md-4 ">
					<div class="form-group">
						<label>TANGGAL PENGISIAN SEBELUMNYA</label>
						   <input type="text" placeholder="" id="tanggal_pengisian" name="tanggal_pengisian" class="form-control datepicker" value="'.$tanggal_pengisian.'" disabled>
					</div>
				</div>
				...
				<div class="col-md-5 ">
					<div class="form-group">
						<label>VOLUME TANGKI BBM PENGUKURAN SAAT INI</label>
						   <input type="text" placeholder="" id="volume_pemakaianx" name="volume_pemakaianx" class="form-control angka" onkeypress="validate(event)" onkeyup="getPenggunaanx()" value="'.number_format($list->volume_sisa).'" readonly="">
					</div>
				</div>
			...
			<div class="row">
				...
				<div class="col-md-5 ">
					<div class="form-group">
						<label>PERUNTUKAN</label>
						<input type="text" name="peruntukan" name="peruntukan" class="form-control" value="'.$list->peruntukan.'" readonly="">
					</div>
				</div>
