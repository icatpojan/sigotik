...
class Ba_peminjaman_bbm extends CI_Controller {
	...
	public function getDataTable(){
		...
		$sql = "SELECT * 
					FROM bbm_kapaltrans a WHERE status_ba = 10
	...
	public function getDataPinjamanBbm(){
		...
		foreach ($query->result() as $list){
			...
			$data['volume_pengisian'] = $list->volume_pengisian;
			
			// $data['jabatan_staf_pangkalan'] = $list->jabatan_staf_pangkalan;
			// $data['nama_petugas'] = $list->nama_staf_pagkalan;
			// $data['nip_petugas'] = $list->nip_staf;
			...
			$data['tanggal_surat'] = $list->tanggal_surat;
	...
	function getDataBa(){
		...
		// var_dump("SELECT * FROM `bbm_kapaltrans` 
		            // JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
		    // WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (2) AND link_modul_ba = '' AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." 
		    // ORDER BY tanggal_surat DESC, jam_surat DESC LIMIT 1");die();
		$query = $ci->db->query($sql);
		...
		if($total>0){
			...
			$data['jml'] = 1;
		    $data['nomor_surat'] = $list->nomor_surat;
		    $data['volume_sisa'] = number_format($list->volume_sisa);
	...
	function getDataBaPinjaman(){
    	...
    	$sql = "SELECT * FROM `bbm_kapaltrans` 
		            JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
		    WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (2) AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." 
		    ORDER BY tanggal_surat DESC, jam_surat DESC LIMIT 1";
		...
		if($total>0){
			...
			$sql2 = "SELECT * FROM `bbm_kapaltrans` 
						JOIN `m_kapal` ON `m_kapal`.code_kapal = `bbm_kapaltrans`.kapal_code
				WHERE tanggal_surat = '".$tanggal_surat."' AND status_ba IN (3) AND m_kapal.m_kapal_id = ".$_POST['m_kapal_id']." AND link_modul_ba = '".$nomor_surat."'
				ORDER BY tanggal_surat DESC, jam_surat DESC LIMIT 1";
			...
			$total2 = $query2->num_rows();
		        ...
		        if($total2>0){
		            ...
		            $data['jml'] = 0;
        		    $data['nomor_surat'] = '';
        		    $data['pesan'] = 'Hari ini anda belum melakukan penggunaan bbm';
                	$data['volume_sisa'] = 0;
		    ...
		    $data['jml'] = 0;
		    $data['nomor_surat'] = '';
		    $data['pesan'] = 'Hari ini anda belum melakukan sounding/pengukuran';
        	$data['volume_sisa'] = 0;
	...
	function getDataEdit($trans_id){
		...
		$ed .= '
			...
			<div class="row">
				...
				<div class="col-md-5">
					<div class="form-group">
						<label>Kapal Peminjam</label>
						<select class="form-control custom-control" id="m_kapal_id_peminjam" name="m_kapal_id_peminjam" onChange="getValKapalPeminjam(this, 1)" '.$cek2.'>';
				...
				<div class="col-md-4 ">
					<div class="form-group">
						<label>KODE KAPAL Peminjam</label>
			...
			<div class="row">
				...
				<div class="col-md-4">
				 ...
				 <div class="col-md-5">
					<label>Jumlah BBM Di Pinjamkan</label>
			...
			<div class="row">
				 ...
				 <div class="col-md-4 ">
					<label>NAMA NAKHODA Peminjam &nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_nakhoda_temp" name="an_nakhoda_temp" value="1" '.$an_nakhoda_temp.' > An.  </label>
				 ...
				 <div class="col-md-4 ">
					<label>Pangkat/Gol </label>
						<input type="text" placeholder="" id="pangkat_nahkoda_temp" name="pangkat_nahkoda_temp" class="form-control" value="'.$list->pangkat_nahkoda_temp.'">
				 </div>
			...
			<div class="row">	
				 ...
				 <div class="col-md-4 ">
					<label>NAMA KKM Peminjam&nbsp;&nbsp;| &nbsp;&nbsp;<input type="checkbox" class="custom-control-input" id="an_kkm_temp" name="an_kkm_temp" value="1" '.$an_kkm_temp.' > An. </label>
	...
	function getDataLihat($trans_id){
		...
		$ed .= '
			...
			<div class="row">
				 ...
				 <div class="col-md-2 ">
					<label>JAM </label>
					<input type="time" name="jam_surat" id="jam_surat" class="form-control" value="'.$list->jam_surat.'" disabled>
				 </div>
			...
			<div class="row">
				...
				<div class="col-md-4 ">
					<div class="form-group">
						<label>KODE KAPAL Peminjam</label>
						<input type="text" name="kapal_code_tempEd" id="kapal_code_tempEd" readonly="" class="form-control" value="'.$list->kapal_code_temp.'" disabled>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-5">
					<div class="form-group">
						<label>Berdasarkan persetujuan</label>
						<select class="form-control custom-control" id="m_persetujuan_id" name="m_persetujuan_id" disabled>';
				...
				<div class="col-md-3 ">
					<div class="form-group">
						<label>NOMOR PERSETUJUAN</label>
						<input type="text" name="nomer_persetujuan" id="nomer_persetujuan" class="form-control" value="'.$list->nomer_persetujuan.'" readonly="">
					</div>
				</div>	
				
				<div class="col-md-3 ">
					<label>TANGGAL PERSETUJUAN</label>
					<input type="text" name="tgl_persetujuan" id="tgl_persetujuan" class="form-control datepicker" value="'.date("d-m-Y", strtotime($list->tgl_persetujuan)).'" disabled>
				 </div>
			...
			<div class="row">
				...
				<div class="col-md-4">
				 ...
				 <div class="col-md-4">
					<label>Sisa BBM</label>
					<input type="text" name="volume_sisaEd" id="volume_sisaEd" class="form-control angka" onkeypress="validate(event)" readonly="" value="'.number_format($list->volume_sisa).'" readonly="">
				 </div>
			</div>
			...
			<div class="row">
			   ...
			   <div class="col-md-7 ">
					<div class="form-group">
						<label>Alasan Peminjaman BBM</label>
						<textarea id="sebab_temp" name="sebab_temp" rows="3" cols="50" class="form-control" readonly="">'.$list->sebab_temp.'</textarea>
					</div>
				</div>
			...
			<br>
				 ...
				 <div class="col-md-4 ">
					<label>Pangkat/Gol </label>
						<input type="text" placeholder="" id="pangkat_nahkoda_temp" name="pangkat_nahkoda_temp" class="form-control" value="'.$list->pangkat_nahkoda_temp.'" readonly="">
				 </div>
